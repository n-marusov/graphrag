...
> **Статус:** этап видения (Фаза 0) → фронтенд реализован (Vue 3 SPA, `src/frontend/`); серверная кодовая база (Go) — план. Архитектура описывает **целевое состояние** (`target`/`to be`); после появления кода диаграммы актуализируются до `as-is` (код — источник истины). Источники истины: `specs/c4/*` (C4-диаграммы), `specs/adr/*` (13 принятых ADR), `specs/vision.md` (канон функций F1–F4), `specs/glossary.md` (единый язык).
