...
sequenceDiagram
    ...
    participant Core as Ядро (ArtifactGraph)

    User->>Core: Изменяемый артефакт (или MR diff)

    %% --- 1. Локализация ---
    Note over Core: Локализация артефакта в ArtifactGraph
...
**Основной поток:**
1. Пользователь указывает изменяемый артефакт (или MR diff → набор артефактов).
2. Ядро локализует артефакт в графе связей артефактов (ArtifactGraph).
