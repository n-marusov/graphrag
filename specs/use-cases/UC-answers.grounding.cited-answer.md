...
sequenceDiagram
    ...
    QE->>KG: Retrieval: релевантные чанки и пути (PPR / PathRAG, без LLM)
    KG-->>QE: Кандидаты: чанки, сущности, пути
...
**Основной поток:**
1. Пользователь задаёт конкретный вопрос → `QueryReceived`.
2. Query Engine нормализует и классифицирует запрос (граница F1.1/F1.2 — тип вопроса, а не качество ответа).
...
6. Query Engine валидирует ссылки: каждая ссылка существует и поддерживает факт.
7. Ответ возвращается пользователю с grounding и provenance → `QueryAnswered`.
...
**Постусловия:** пользователь получил проверяемый ответ со ссылками на артефакты; каждый факт трассируется до документа или чанка (K1).
...
**Целевые примечания:** NFR-ACC-1 — точность ответов RAG не ниже 70% на утверждённом benchmark-наборе (LLM-as-judge) — целевая метрика Фазы 1.5 (vision §2.6), не гарантия основного потока.
