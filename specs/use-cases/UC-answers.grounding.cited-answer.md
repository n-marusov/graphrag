...
**Основной поток:**
1. Разработчик задаёт конкретный вопрос → `QueryReceived`.
...
7. Ответ возвращается разработчику с grounding и provenance → `QueryAnswered`.
...
**Постусловия:** разработчик получил проверяемый ответ со ссылками на артефакты; каждый факт трассируется до документа или чанка (K1).
