...
- **Акторы** — архетипы пользователей: команда разработки, эксплуатация и безопасность, программные пользователи (ИИ-агенты через MCP). Полный список профилей — `vision.md` §3.1.
- **Внешние системы** — GitLab (эталонный источник, включая репозиторий модели предметной области), внешнее хранилище документов (книги и легаси, глобальный контекст), контур LLM, инфраструктура мониторинга и аудита.
...
- **Источник**: канонический контекст — `specs/vision.md` §3.3; диаграмма расходится с §3.3: роли сгруппированы в архетипы (полные профили — vision §3.1), добавлены внешние зависимости из §2.8 (внешнее хранилище документов, мониторинг и аудит); расхождения по отдельным элементам — см. ниже.
