...
7. **Сопровождение (P2, домен ЖЦ)** — Администратор: мониторинг золотых сигналов (`US-ops.monitor.health`), обновление/откат (`US-ops.update.rollback`), восстановление из бэкапа (`US-ops.restore.from-backup`).
8. **Вывод из эксплуатации (P2, домен ЖЦ)** — Администратор: остановка инстанса (`US-decommission.stop.instance`), удаление данных/томов (`US-decommission.remove.data`); исходные данные в GitLab/S3 не затрагиваются.
...
| Контур LLM | Внешняя система (исполнение моделей) | Open-weight модели на локальных GPU (основной вариант) и/или внешний провайдер по договору через корпоративный шлюз | Local / корпоративный шлюз | Извлечение, генерация, оценка (REQ-NFR-security.compliance.llm-contour) |
