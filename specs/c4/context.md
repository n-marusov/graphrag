...
| IT-безопасность | Человек | Аудит контура LLM; фильтрация утечек | NFR-SEC-1 | API |
...
| Контур LLM | Внешняя система (исполнение моделей) | Open-weight модели на локальных GPU | Local | Извлечение, генерация, оценка (NFR-SEC-1) |
...
- **Контур LLM — чёрный ящик**: на уровне контекста показывается только внешний контракт (Local); детали контура, фильтрация утечек и аудит (NFR-SEC-1) — уровни 2–3 и ADR.
