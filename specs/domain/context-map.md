...
5. **Контур LLM.** Данные разработки (код, спеки) не покидают периметр; система не обрабатывает ПДн как бизнес-данные (vision §2.7 п.12); фильтрация утечек и аудит обращений обязательны (REQ-NFR-security.compliance.llm-contour, NFR-SEC-1).
