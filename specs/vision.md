...
|---|---|---|---|
| **1** | **F1. Проверяемые ответы по проекту** | Вопросы команды по спецификациям, коду и продукту закрываются проверяемыми ответами со ссылками на артефакты; строятся ответы на сложные вопросы по нескольким артефактам. | Быстрые и проверяемые ответы без ожидания аналитика; понимание кода и связей «требование → реализация». |
| **2** | **F2. Актуальная база знаний** | Изменения в GitLab (спеки и код) и загрузки PDF автоматически попадают в базу знаний. | Знания всегда соответствуют текущему состоянию проекта; обновление без полной перестройки. |
...
|---|---|---|---|
| **F1** | Проверяемые ответы по проекту | G1, G3 | Ответы со ссылками на артефакты, глобальные и multi-hop вопросы |
| **F2** | Актуальная база знаний | G2 | Индексация изменений GitLab (спеки и код) и PDF |
...
| F1.2 | Целостная картина вместо «лоскутов» | Аналитик, Архитектор | Как аналитик, я хочу получать ответы, требующие связывания фактов из разных артефактов (темы, тренды, цепочки), чтобы видеть продукт целиком |
...
flowchart LR
    ...
    F1 --> F1_2["F1.2 Глобальные и multi-hop вопросы"]
