...
> **Канон нумерации функций.** Функции уровня 1 — F1–F4 (данный документ) — **единый канон** для всех артефактов проекта (`src/`, `qa/`, `adr/`, `domain/`). Механизмы (онтологический слой, контур LLM, инкрементальное обновление) и сквозные требования (`NFR-*`, §2.5) функциями не являются и F-кодов не получают. Ранее использовавшаяся в тест-планах и QA-артефактах схема F1–F7 (функции + механизмы) отменена; маппинг старой схемы на канон — `specs/open-questions.md` Q1.1.
...
**Сквозные требования (входят в MVP, не являются функциями; идентификаторы — для трассируемости и модификации):**
- **NFR-SEC-1.** Безопасный контур LLM: локальные GPU, open-weight модели; данные не покидают периметр; фильтрация утечек и аудит обращений.
- **NFR-PERF-1.** Минимизация токенов и числа LLM-вызовов как сквозное требование (локальные GPU, ограниченный контекст).
- **NFR-ACC-1.** Точность ответов: порог продолжения внедрения — точность RAG не ниже 70% на утверждённом benchmark-наборе (LLM-as-judge).
- **NFR-PROC-1.** Принцип «агент готовит — человек подтверждает и отвечает»: ответы и артефакты, сгенерированные LLM, подтверждаются человеком.
- **NFR-MOD-1.** Модель предметной области (машинно-читаемая онтология) — механизм внутри F1 и F3; исходная модель поставляется из репозитория модели, автоматическое построение из корпуса — Фаза 3.
- **NFR-IMPL-1.** Принципы внедрения: метрики пользы, внедрение малыми шагами.
