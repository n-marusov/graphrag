...
| **2** | **F2. Индексация источников знаний** | Изменения в GitLab (спеки и код) и загрузки PDF автоматически попадают в базу знаний. | Знания всегда соответствуют текущему состоянию проекта; обновление без полной перестройки. |
| **3** | **F3. Вычисление влияния и компиляция контекста** | Изменение артефакта разворачивается в список связанных артефактов, требующих изменений; для генерации собирается JSON-контекст из связанных артефактов с направлениями связей. | Ничего не пропускается при изменениях; генерация артефактов идёт по полному, но сфокусированному контексту — экономия токенов. |
...
| **F2** | Индексация источников знаний | G2 | Актуализация базы знаний по изменениям в GitLab и загрузкам PDF |
| **F3** | Вычисление влияния и компиляция контекста | G4 | Последствия изменения артефакта и JSON-контекст для генерации |
...
|---|---|---|---|
| F1.1 | Генерация локальных ответов со ссылками на артефакты | Разработчик, Аналитик | Как разработчик, я хочу получать ответ на вопрос по продукту, спецификации или коду со ссылками на конкретные артефакты, чтобы проверять его самому и не ждать аналитика |
| F1.2 | Синтез глобальных ответов | Аналитик, Архитектор | Как аналитик, я хочу получать ответы, требующие связывания фактов из разных артефактов, чтобы видеть продукт целиком |
...
| F2.2 | Загрузка PDF-книг и легаси | Аналитик | Как аналитик, я хочу загружать PDF-книги и унаследованные документы, чтобы их знания были доступны в системе |

##### F3. Вычисление влияния и компиляция контекста

| Код | Функция (ценность для пользователя) | Роль | Формулировка US |
|---|---|---|---|
| F3.1 | Вычисление последствий изменения | Разработчик, Архитектор | Как разработчик, я хочу при изменении артефакта получать список связанных артефактов, которые потребуют изменений, чтобы учитывать все последствия |
...
> **Граница F4.** MCP — интерфейс доступа к функциям F1 и F3 для ИИ-агентов. Обновление базы знаний (F2) — детерминированная задача и не исполняется через MCP: изменения приходят из GitLab и загрузок PDF. F4 не дублирует содержимое F1 и F3 — он описывает канал доступа.
...
flowchart LR
    ...
    Root --> F3["F3: Вычисление влияния и компиляция контекста"]
    Root --> F4["F4: Доставка через MCP"]

    F1 --> F1_1["F1.1 Генерация локальных ответов"]
    F1 --> F1_2["F1.2 Синтез глобальных ответов"]

    F2 --> F2_1["F2.1 Автоиндексация GitLab"]
    F2 --> F2_2["F2.2 Загрузка PDF"]

    F3 --> F3_1["F3.1 Вычисление последствий"]
...
|---|---|---|---|---|
| `DocumentIngested` | Новый/изменённый документ в GitLab (MR) или загрузка PDF | Репозиторий GitLab / загрузка PDF | Индексация | Подготовка документа к включению в базу знаний |
...
**Назначение MVP.** MVP подтверждает базовый контур инструмента: корпус (спеки и код из GitLab, PDF) индексируется в граф знаний, команда получает проверяемые ответы со ссылками на артефакты, изменения в GitLab и загрузки PDF автоматически актуализируют базу знаний, а ИИ-агенты получают точный контекст через MCP. Vision описывает только продуктовые возможности и границы.

**F1. Генерация проверяемых ответов (MVP):**
- Генерация локальных ответов на вопросы по спецификациям и коду со ссылками на конкретные артефакты.
- Синтез глобальных и multi-hop ответов в базовом контуре; полный контур — Фаза 2.

**F2. Индексация источников знаний (MVP):**
- Актуализация базы знаний по изменениям в GitLab.
- Актуализация базы знаний по загруженным PDF и унаследованным документам.

**F3. Вычисление влияния и компиляция контекста (MVP):**
- Вычисление последствий изменения: детерминированный список связанных артефактов, требующих изменений.
- Компиляция контекста: JSON-объект — набор связанных артефактов (спеки, код, ADR) с направлениями связей; критерий — полнота при минимуме токенов.
...
**Что НЕ входит в MVP:**
- Полноценный контур глобальных вопросов и продвинутые графовые методы — Фаза 2/3.
...
- Генерация проверяемых локальных ответов по спекам и коду; синтез базовых глобальных ответов; индексация источников знаний.
- Вычисление последствий изменения и компиляция контекста для генерации.
...
- Полный контур глобальных вопросов и multi-hop сценариев.
...
| **Функции** | Ограничение (MVP) | MVP: базовые функции всех 4 функций (F1–F4). Фаза 2: полный контур глобальных вопросов, расширение компиляции контекста. Фаза 3: агентное извлечение, обновление в реальном времени, автоматическое построение модели предметной области. |
