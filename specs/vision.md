...
| **2** | **F2. Индексация источников знаний** | Изменения источников знаний автоматически попадают в базу знаний: GitLab (спеки, код, онтологии, структурированные данные) и внешнее хранилище документов (книги и легаси). | Знания всегда соответствуют текущему состоянию проекта; обновление без полной перестройки. |
...
| Код | Функция (ценность для пользователя) | Роль | Формулировка US |
|---|---|---|---|
| F2.1 | Автоиндексация изменений GitLab | Разработчик, Аналитик | Как разработчик, я хочу, чтобы изменения артефактов в GitLab (спеки, код, онтологии, структурированные данные) автоматически попадали в базу знаний, чтобы ответы не устаревали |
| F2.2 | Индексация книг и легаси из внешнего хранилища | Аналитик, Разработчик | Как аналитик, я хочу, чтобы книги и унаследованные документы (PDF) из внешнего хранилища автоматически попадали в базу знаний, чтобы глобальные знания были доступны в системе |
...
- Актуализация базы знаний по изменениям в GitLab: спеки, исходный код, онтологии, структурированные данные.
...
|---|---|---|
| Репозиторий GitLab (спеки, код, онтологии, структурированные данные) | Authoritative-источник корпуса, триггеры индексации | Git API / webhook / MR |
...
| Модель предметной области | Исходная модель (онтология) для резолвинга и контекста | Репозиторий модели |
...
C4Context
    ...
    Enterprise_Boundary(org, "Организация", "boundary") {
        ...
        System_Ext(gitlab, "GitLab", "Эталонный источник: спеки, код, онтологии, структурированные данные, MR, владельцы")
    ...
    BiRel(graphRag, gitlab, "Индексация артефактов целевого проекта", "Git API / MR / webhook")
