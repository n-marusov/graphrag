...
| G4 — Онбординг | Новичок закрывает вопросы по продукту через систему знаний | Время до первого принятого MR — значение фиксируется после замера исходного уровня |
| G5 — Безопасность | Обращения к LLM — только через корпоративный контур | Доля обращений через корпоративный канал; 0 обращений вне контура — значение фиксируется после замера исходного уровня |
| G6 — Рутина | Типовые вопросы не требуют участия аналитика | Доля вопросов, закрытых системой знаний — значение фиксируется после замера исходного уровня |
...
|---|---|---|---|
| **F1** | Проверяемые ответы по проекту | G1, G2 | Ответы со ссылками на артефакты, поиск реализации требований в коде, глобальные и multi-hop вопросы |
| **F2** | Актуальная база знаний | G1 | Индексация изменений GitLab (спеки и код) и PDF, инкрементальное обновление, управление обновлением |
| **F3** | Связи артефактов и контекст для изменений | G3 | Граф зависимостей артефактов, анализ влияния изменений, выборка контекста для генерации |
| **F4** | MCP — интерфейс для ИИ-агентов | G1, G3 | Стандартный доступ ИИ-агентов к ответам и контексту |
...
- Модель предметной области (машинно-читаемая DDD-модель: ограниченные контексты, единый язык, агрегаты, доменные события) — механизм внутри F1 и F3; в MVP ведётся кураторами вручную.
...
- Автоматическое построение и поддержание модели предметной области с кураторами.
...
| Модель предметной области | Исходная DDD-модель для резолвинга и контекста | Модель/репозиторий |
...
| A4 | Модель предметной области ведётся кураторами, а не строится автоматически в MVP | Термины и связи разъезжаются, контекст растёт |
...
| **Аналитик (в команде)** | Бизнес-знания собраны в системе; ответы на нетиповые вопросы; контур сигналов вместо охоты за расхождениями. | Прямой пользователь | F1, F2, F3 | Поддерживает модель предметной области; не отвечает на повторяющиеся вопросы вручную. |
...
| **Инкрементальное обновление** | **Критический фактор** | Корпус меняется через MR и загрузки PDF; полная перестройка на каждое изменение недопустима по стоимости. [F2] |
| **Безопасность контура LLM** | **Критический фактор** | Данные разработки (код, спеки, ПДн) не покидают периметр; фильтрация утечек и аудит — обязательны. [G5] |
| **Трассируемость и анализ влияния** | **Высокий приоритет** | Детерминированный анализ влияния — основа постановки задач и сверки «результат ↔ источник». [G3, F3] |
