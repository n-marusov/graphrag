...
|---|---|
| Основная база | Функции F1–F7 и критерии приёмки — `../vision.md` §2.1–2.2 (каталог требований планируется, §2.2); цели G1–G7 и метрики — §1.3 |
| Объект | Go-пакеты модулей GraphRAG: ядро, оркестрация пайплайнов (F1), query engine (F3), MCP-сервер (F6), интеграции (GitLab, GPU-контур), CPU-компоненты (токенизатор, HNSW, Leiden) — состав в `../src/README.md` |
...
1. **Требование реализовано?** Критерий приёмки функции F1–F7 (каталог требований планируется, `../vision.md` §2.2) выражен проверкой.
...
|---|---|
| P0 | Критичный путь, безопасность контура LLM (фильтрация утечек, аудит — F7), онтологический резолвинг (F2), извлечение знаний (F1), анализ влияния (F4), целостность графа, сохранность данных, статусы пайплайнов |
| P1 | Основная бизнес-логика функций F1–F7, retry/timeout/idempotency-логика, маппинг контрактов (GitLab API, MCP JSON-RPC), use case поведение |
...
|---|---|---|
| Ядро (граф знаний, хранилище) | модели сущностей/связей/утверждений, чанкинг, mutual indexing факт↔чанк (F1.5), статусы пайплайнов, storage-адаптеры через изоляцию | чанкинг markdown/PDF, валидация чанка/сущности/утверждения, дубликаты |
| Оркестрация пайплайнов (F1) | автоиндексация (F1.1), инкрементальное обновление (F1.3), обработка ошибок зависимостей (GitLab API/webhook), retry/idempotency | инкрементальное обновление, повторная индексация |
| Query engine (F3) | dual-level retrieval (LightRAG), pruning путей (PathRAG), PPR, multi-hop (F3.3), детекция противоречий (F3.4), LLM-as-judge (F3.5) | ранжирование, детекция противоречий, сборка ответа с grounding |
| Онтологический слой (F2) | резолвинг терминов по DDD-модели (F2.1), единый язык (F2.2), сигналы куратора (F2.3), минимизация контекста (F2.4) | резолвинг термина, unknown term, конфликты терминов |
| MCP-сервер (F6) | сборка контекста (F6.1), схема MCP-инструментов (JSON-RPC), сверка со спецификациями (F6.2), предзавершающий гейт (F6.3), QA-контекст (F6.4) | валидация JSON-RPC, лимиты токенов |
...
Критичные модули качества, unit-покрытие которых обязательно в первую очередь (приоритет P0, раздел 3.1): контур LLM (фильтрация утечек, аудит — F7), онтологический слой (резолвинг — F2), извлечение знаний (F1), анализ влияния (детерминизм — F4), MCP-компилятор (F6), CPU-компоненты (токенизатор, HNSW, Leiden), инкрементальное обновление графа (F1.3).
...
- security-ограничения: отсутствие секретов и данных корпуса в логах/аудит-выводах, обращение к LLM только через корпоративный контур (F7.2), запрет open-by-default поведения;
...
1. **Требование** — функции F1–F7 и подфункции из `../vision.md` §2.1–2.2 (каталог требований планируется, §2.2), цели G1–G7 и метрики из §1.3, или сценарий из `qa/test-plan.md`.
...
1. Найти функцию/подфункцию F1–F7, которая реализуется изменением (`../vision.md` §2.1–2.2); каталог требований с критериями приёмки планируется (§2.2).
...
7. Добавить комментарий трассировки в формате `// F<номер>.<подфункция>: <описание>` (например, `// F1.3: ...`) над тестом или группой subtest (раздел 6.6). Если тест покрывает дефект без отдельной функции, указать ID дефекта/регрессии и связанную функцию при наличии.

### 5.2. Шаблон декомпозиции критерия

```text
Требование: F<номер>.<подфункция> (например, F1.3)
...
```text
Требование: инкрементальное обновление графа знаний при появлении нового документа (F1.3).
...
- OntologyResolver.Resolve(термин) возвращает канонический термин DDD-модели
- неизвестный термин возвращает ошибку/unknown term

Integration:
- webhook GitLab → оркестрация пайплайна F1 → граф обновлён инкрементально без полной переиндексации.

E2E API:
- пользователь получает ответ со ссылкой на источник (grounding, F3.1).
```
...
Каждый unit-тест или логическая группа table-driven subtest должна содержать комментарий трассировки в формате `// F<номер>.<подфункция>: <описание>` (F-код функции/подфункции из `../vision.md` §2.1–2.2, например `// F1.3: ...`), где `<описание>` — кратко, какое правило проверяется.

```go
// F1.3: инкрементальное обновление графа при появлении нового документа.
...
- **ID должен существовать** в `../vision.md` (F1–F7, §2.1–2.2) или в каталоге требований, когда он появится (планируется, §2.2). Несуществующий или изменённый ID — дефект трассировки, который должен ловить гейт `FG-TRACE`.
...
## 8. Специфика по функциям проекта (F1–F7)

Ниже — типовые unit-проверки и обязательные negative cases по функциям проекта (F1–F7, `../vision.md` §2.1–2.2). Для подфункций, где деталей пока нет, указано «(детали уточняются при реализации)».

### 8.1. F1: индексация корпуса и граф знаний

Unit-проверки:

- чанкинг markdown-спек GitLab и PDF-книг (RAPTOR/StructRAG, F1.4) на уровне чистых функций;
- маппинг ответа LLM-извлечения в доменные модели сущностей/связей/утверждений (F1.2);
- mutual indexing факт ↔ чанк (KAG-принцип, F1.5);
- инкрементальное обновление: обработка только изменённых документов (F1.3);
...
### 8.2. F2: онтологический слой

Unit-проверки:

- резолвинг терминов запроса к каноническим терминам DDD-модели (F2.1);
- единый язык: маппинг синонимов/сокращений в канонический термин (F2.2);
- обработка сигналов куратора об изменении онтологии (F2.3);
- минимизация контекста: отбор релевантных артефактов/подграфа (F2.4);
- загрузка и валидация машинно-читаемой DDD-модели (онтологии).
...
- сигнал куратора со ссылкой на несуществующий артефакт.

### 8.3. F3: ответы с привязкой к источнику

Unit-проверки:

- dual-level retrieval (LightRAG): глобальный и локальный отбор, ранжирование (F3.2);
- pruning путей (PathRAG) и PPR — детерминированные вычисления без LLM на извлечении;
- multi-hop: композиция промежуточных выводов (F3.3);
- детекция противоречий между источниками (F3.4);
- LLM-as-judge: оценка точности ответа, маппинг вердикта (F3.5);
- сборка ответа со ссылками на источники (grounding, F3.1).
...
### 8.4. F4: трассируемость и анализ влияния

Unit-проверки:

- граф связей артефактов (документ → чанк → сущность → утверждение → ответ);
- детерминированный анализ влияния: транзитивное замыкание по графу (F4.1);
- сверка «результат ↔ источник» (F4.2);
- построение графа до клиентского результата (F4.3);
- формирование сигнала владельцу затронутого артефакта (F4.4).
...
### 8.5. F5: накопление знаний

Unit-проверки:

- структурирование сигналов с полей в доменные события (`SignalReceived`, `KnowledgeRefreshed` — `../vision.md` §2.4, F5.1);
- привязка сигнала к сценарию/функции/артефакту;
- разбор инцидентов (F5.2);
- обновление знаний из прода/телеметрии (F5.4);
- метрика времени «сигнал → фикс» (F5.3) на уровне чистой логики.

Обязательные negative cases:

- сигнал без обязательных полей (неизвестный источник, пустой текст);
- дубликат сигнала (идемпотентность);
- привязка к несуществующему артефакту (детали уточняются при реализации).

### 8.6. F6: context compiler MCP

Unit-проверки:

- сборка точного набора артефактов для ИИ-агента (F6.1);
- схема MCP-инструментов и JSON-RPC payload (F6.2);
- сверка собранного контекста со спецификациями;
- предзавершающий гейт: проверка полноты/валидности контекста перед выдачей агенту (F6.3);
- QA-контекст (F6.4).
...
### 8.7. F7: безопасный контур LLM

Unit-проверки:

- фильтрация утечек: запрет выдачи секретов/внутренних данных в контекст и ответы (F7.2);
...
- минимизация токенов/LLM-вызовов: правила отбора контекста (F7.3).
...
Порог покрытия **не зафиксирован (TBD)**: фиксируется вместе с первым кодом; NFR-каталог планируется (`../vision.md` §2.2). Целевой CI публикует `coverage.out` и cobertura-отчёт.
...
- контура LLM и фильтрации утечек (F7);
- онтологического слоя (резолвинг — F2);
- извлечения знаний (F1);
- анализа влияния (детерминизм — F4);
- MCP-компилятора (F6);
- инкрементального обновления графа (F1.3);
...
В проекте несколько Go-модулей (ядро, оркестрация пайплайнов, query engine, MCP-сервер, интеграции, CPU-компоненты), поэтому команды запускаются из директории конкретного модуля. Каталоги модулей фиксируются с первым кодом (состав — `../src/README.md`); ниже `<module>` — имя каталога модуля, `<package>` — имя тестируемого пакета.
...
- [ ] Каждый тест или группа subtest содержит комментарий трассировки в формате `// F<код>: ...` (например, `// F1.3: инкрементальное обновление графа`) — один ID на строку, ID существует в `../vision.md` (F1–F7, §2.1–2.2) или в каталоге требований, когда он появится (основа `FG-TRACE`).
...
- [ ] Комментарий трассировки в формате `// F<код>: ...` (например, `// F1.3: ...`), один ID на строку; ID существует в `../vision.md` или каталоге требований (когда он появится).
...
1. Определи модуль: ядро, оркестрация пайплайнов (F1), query engine (F3), MCP-сервер (F6), интеграции (GitLab, GPU-контур), CPU-компоненты (токенизатор, HNSW, Leiden) — состав в `../src/README.md`.
...
1. Прочитай связанные требования: функции F1–F7 и подфункции из `../vision.md` §2.1–2.2, цели и метрики из §1.3, сценарии из `qa/test-plan.md`; каталог требований планируется (vision.md §2.2).
...
Requirement/Reason: <F-код (например, F1.3)/bug/regression>
...
- Добавь обязательный комментарий трассировки над тестом или группой subtest в формате `// F<код>: <описание>` (например, `// F1.3: ...`) — один ID на строку (раздел 6.6).
...
- `../vision.md` — видение: функции F1–F7 (§2.1–2.2), цели G1–G7 (§1.3), метрики, события (§2.4), внешние зависимости (§2.8).
...
- `../../.agents/skills/aif-security-checklist/SKILL.md` — security-аудит (F7).
...
- каталог функциональных требований F1–F7 с критериями приёмки;
