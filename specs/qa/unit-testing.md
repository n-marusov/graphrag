...
| Объект | Go-пакеты модулей GraphRAG: ядро, оркестрация пайплайнов (F2), query engine (F1), MCP-сервер (F4), интеграции (GitLab, GPU-контур), CPU-компоненты (токенизатор, HNSW, Leiden) — состав в `../src/README.md` |
...
|---|---|
| P0 | Критичный путь, безопасность контура LLM (фильтрация утечек, аудит — NFR-SEC-1), онтологический резолвинг (механизм F1/F3), извлечение знаний (F2), анализ влияния (F3), целостность графа, сохранность данных, статусы пайплайнов |
...
| Query engine (F1) | dual-level retrieval (LightRAG), pruning путей (PathRAG), PPR, multi-hop (F1.2), детекция противоречий (G3), LLM-as-judge (NFR-ACC-1) | ранжирование, детекция противоречий, сборка ответа с grounding |
| Онтологический слой (механизм F1/F3) | резолвинг терминов по модели, канонические термины, обновление модели, минимизация контекста (NFR-PERF-1) | резолвинг термина, unknown term, конфликты терминов |
...
- security-ограничения: отсутствие секретов и данных корпуса в логах/аудит-выводах, обращение к LLM только через корпоративный контур (NFR-SEC-1), запрет open-by-default поведения;
...
- минимизация контекста: отбор релевантных артефактов/подграфа (NFR-PERF-1);
...
- LLM-as-judge: оценка точности ответа, маппинг вердикта (NFR-ACC-1);
...
### 8.7. Сквозное требование: безопасный контур LLM (NFR-SEC-1)

Unit-проверки:

- фильтрация утечек: запрет выдачи секретов/внутренних данных в контекст и ответы (NFR-SEC-1);
...
- минимизация токенов/LLM-вызовов: правила отбора контекста (NFR-PERF-1).
...
- контура LLM и фильтрации утечек (NFR-SEC-1);
...
1. Определи модуль: ядро, оркестрация пайплайнов (F2), query engine (F1), MCP-сервер (F4), интеграции (GitLab, GPU-контур), CPU-компоненты (токенизатор, HNSW, Leiden) — состав в `../src/README.md`.
...
- `../../.agents/skills/aif-security-checklist/SKILL.md` — security-аудит (NFR-SEC-1).
