...
| Query engine (F1) | dual-level retrieval (LightRAG), pruning путей (PathRAG), PPR, multi-hop (F1.2), детекция противоречий (G3), LLM-as-judge (REQ-NFR-api.compliance.rag-accuracy) | ранжирование, детекция противоречий, сборка ответа с grounding |
