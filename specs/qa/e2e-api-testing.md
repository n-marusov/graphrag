...
3. Проверить контрактные тесты эталонных кейсов K1/K4/K5/K17 (каноническая команда — `go test -race -v -tags=e2e ./...`; см. §17.1).
...
Полные описания — в `qa/test-plan.md` §7. Ниже свод идентификаторов с уровнем `E2E API`. Идентификаторы `F1-01`…`F4-07` (плюс `ONT-*`, `SEC-*`) — заготовки: финальные scenario ID ведутся в `qa/test-plan.md` §7.
...
Контрактные тесты эталонных кейсов — K1 (ответ с grounding), K4 (анализ влияния), K5 (сверка результат↔источник), K17 (context compiler) — пишутся до реализации функций (контрактный уровень). Каноническая команда целевого состояния для автоматизированного набора:

```sh
go test -race -v -tags=e2e ./...
```

До появления кода допускается ручной contract-review по шаблону §17.3; результат фиксируется как `not implemented`/`not verified`, но отсутствие реализации не является открытым вопросом требований.
