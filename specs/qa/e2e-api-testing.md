...
**Правило источников.** E2E-сценарии выводятся **только из функций проекта** (`../vision.md` §2.1–2.2, подпункты F1.1–F7.3). Каталоги требований (планируются, vision.md §2.2), контракты, ADR и glossary — контекст: они уточняют шаги, payload и assertions, но не порождают E2E-сценарии. Если для пути нет критериев функции — уровень не E2E API, а integration (до появления детализированных требований).
...
|---|---|
| Основная база | `../vision.md` §2.2 — F1.1–F7.3: основной источник сценариев; каталоги требований (планируются, vision.md §2.2), контракты, ADR, glossary — контекст |
| Объект | Полные пути через ядро, пайплайны индексации (F1), онтологический слой (F2), query engine (F3), анализ влияния (F4), MCP-сервер (F6), контур LLM (F7) и тестовый контур по контрактам HTTP API / MCP (JSON-RPC 2.0) / GitLab API / LLM-контур / хранилища / шина сигналов |
...
1. **Полный путь работает?** Внешний запрос проходит через все звенья (транспорт → ядро → пайплайн/query engine → компонент → обратный транспорт → тестовый контур) и даёт ожидаемый внешний результат.
...
3. **Бизнес-инварианты держатся?** Правила проекта соблюдаются на всём пути: единый язык терминов (F2.2), grounding и атрибуция источников (F3.1), детерминизм анализа влияния (F4), идемпотентность повторной индексации и повторной доставки сигналов, отсутствие дублей сущностей/утверждений.
...
5. **Путь устойчив?** Обрыв на любом звене (GitLab, LLM-контур, хранилище графа/векторов, шина сигналов, компонент контура) приводит к контролируемому ретраю/отказу/состоянию, а не к потере данных или рассинхрону.
...
| P1 | Основные доменные пути (F1–F7), сквозные негативные сценарии, идемпотентность повторной индексации и повторной доставки сигналов, консистентность состояний, формат ошибок, сквозная телеметрия/аудит |
...
|---|---|
| HTTP API (запросы/ответы, метрики, здоровье) | запросы от команды/клиентов через реальные ядро и query engine, а не с заглушкой на месте контура |
| MCP (JSON-RPC 2.0) | tools/resources/prompts context compiler: компиляция контекста для ИИ-агента реальным MCP-сервером (F6) |
| GitLab API/webhook/MR | индексация корпуса: webhook/MR → DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed (F1) |
...
| Хранилище графа/векторов | согласованность графа и векторного индекса после инкрементального обновления (F1.3); выбор хранилища открыт — ADR-IMPL.DATA.graph-storage |
| Шина сигналов (F5) | SignalReceived → KnowledgeRefreshed; повторная доставка и идемпотентность |
| Мониторинг/аудит | SecurityEventEmitted от реального контура до приёмника (F7.2) |
...
Источник правды для E2E API-сценария — **функция проекта** (`../vision.md` §2.1–2.2, подпункты F1.1–F7.3): сценарий существует, потому что есть критерий целевого поведения. Остальные артефакты — контекст: они уточняют шаги, payload и assertions, но не порождают E2E-сценарии.

**Основной источник (правило отбора):**

1. **`../vision.md` §2.1–2.2** — функции F1–F7 с подпунктами (критерии целевого поведения). Один E2E-сценарий соответствует одной функции (подпункту): вход задаёт шаги сценария, критерий — ожидаемый результат (oracle).
...
3. **`../glossary.md`** — единый язык терминов (F2.2): резолвинг понятий в запросах и артефактах.
4. **Контракты** — HTTP API (запросы/ответы, метрики, здоровье), MCP (JSON-RPC 2.0: tools/resources/prompts), GitLab API/webhook/MR, локальный LLM-контур (OpenAI-совместимый endpoint), хранилище графа/векторов (выбор открыт — ADR-IMPL.DATA.graph-storage), шина сигналов, мониторинг/аудит.
5. **Метрики** (vision.md §1.4, glossary §8): точность ответов RAG ≥ 70 %; время ответа (медиана, p90); число противоречий; доля артефактов со связями; время сигнал→фикс; доля обращений к LLM через корпоративный канал (0 вне контура); минимизация токенов/LLM-вызовов.
6. **`qa/test-plan.md`** — доменные сценарии и уровни; scenario ID (`F1-01`…`F7-03`, `K1/K4/K5/K17`).
...
1. Найти функцию (подпункт F1.1–F7.3, vision.md §2.2) с критерием проверяемого поведения — основной источник E2E-сценария; если критерия нет — это не E2E API (раздел 4).
...
5. Найти контракты всех сегментов: HTTP API, MCP (JSON-RPC 2.0), GitLab API/webhook/MR, локальный LLM-контур (OpenAI-совместимый endpoint), хранилище графа/векторов, шина сигналов, мониторинг/аудит.
...
```go
// F3.1: ответ на запрос содержит ссылки на источники (grounding) — основной trace.
// F3.3: multi-hop — контекст.
// F2.1: резолвинг запроса по DDD-модели — контекст.
...
func TestIndexing_EndToEnd_MRToGraphUpdate(t *testing.T) {
    // F1.1: автоиндексация корпуса по webhook/MR (DocumentIngested → GraphUpdated).
}

func TestContextCompiler_EndToEnd_MCP(t *testing.T) {
    // F6.1: точный контекст агенту через MCP (ContextCompiled).
}
```
...
|---|---|---|---|
| HTTP API | клиент ↔ контур | HTTP-клиент (запрос/ответ, метрики, здоровье) → ядро/query engine → ответ или событие в тестовом контуре | F3-01†, F4-01† |
| MCP (JSON-RPC 2.0) | ИИ-агент ↔ контур | MCP-клиент агента → context compiler (tools/resources/prompts) → компиляция контекста → ответ агенту | F6-01† |
| GitLab API/webhook/MR | GitLab ↔ контур | webhook/MR → пайплайн индексации → граф → события (DocumentIngested → … → CommunityRecomputed) | F1-01† |
| Локальный LLM-контур | контур ↔ GPU | запрос (OpenAI-совместимый endpoint) → эмулятор/GPU → ответ → фильтрация утечек → аудит | F7-01† |
| Хранилище графа/векторов | пайплайны ↔ хранилище | запись/чтение при инкрементальном обновлении → согласованность графа и индексов | F1-03† |
| Шина сигналов (F5) | поля → контур | сигнал → SignalReceived → привязка к сценарию → KnowledgeRefreshed | F5-01† |
| Мониторинг/аудит | контур → приёмник | SecurityEventEmitted → приёмник → подпись, идемпотентность | F7-02† |
...
|---|---|---|
| MR → индексация → граф → ответ со ссылками (F1+F3) | GitLab webhook/MR → пайплайн индексации → хранилище графа → community summaries → query engine → ответ | события DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed; ответ содержит ссылки на источники (grounding) |
| Запрос → резолвинг по онтологии → retrieval → ответ с grounding (F2+F3) | HTTP API → онтологический слой → dual-level retrieval (LightRAG)/PathRAG/PPR → генерация (LLM-контур) → ответ | состав источников и атрибуция согласованы; дубли запросов не меняют состояние |
| Изменение артефакта → анализ влияния → сигнал владельцу (F4) | webhook/MR → пайплайн → граф связей → InfluenceComputed → шина сигналов | список затронутых, сверка результат↔источник, сигнал владельцу (F4.2, F4.4) |
| Сигнал с полей → структурирование → обновление знаний (F5) | шина сигналов → SignalReceived → привязка к сценарию → KnowledgeRefreshed | повторная доставка не дублирует знания; время сигнал→фикс метрицируется |
| Запрос агента → MCP-компиляция контекста → сверка со спецификациями (F6) | MCP-клиент → context compiler → сверка → ContextCompiled → ReviewCompleted | предзавершающий гейт и QA-контекст работают на полном пути |
| Обращение к LLM → фильтрация → аудит (F7) | query engine/пайплайн → контур LLM → фильтр утечек → аудит-журнал → SecurityEventEmitted | утечки блокируются, аудит полный, 0 обращений вне корпоративного канала |
...
| Основной стенд интеграции | Реальные процессы контура GraphRAG (ядро, пайплайны индексации, query engine, MCP-сервер, интеграции, CPU-компоненты) + точечные заглушки внешних стыков | integration-сценарии |
...
- Тестовый контур — **согласованный полный контур** (заглушка GitLab, эмулятор LLM с детерминированными golden-ответами, тестовые хранилища графа/векторов, тестовая шина сигналов, webhook/MR-приёмник, мониторинг/аудит stub), изолированный от прод-подобного.
...
- Стенд допускает fault injection: остановку/отключение любого звена пути (GitLab, LLM-контур, хранилище, шина сигналов, компонента) для негативных сценариев.
...
- проверяем «запрос → ответ с grounding» → реальные ядро и query engine, эмулятор LLM (детерминированные golden-ответы) допустим как генератор на внешнем контуре;
...
| Тестовая шина сигналов | инициатор F5 | сигналы с полей, повторная доставка, дубликаты |
...
|---|---|
| Ядро | оркестрация пайплайнов, события, онтологический слой (F2), анализ влияния (F4), статусы задач |
| Пайплайны индексации (F1) | DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed; PDF-контур RAPTOR/StructRAG; mutual indexing; инкрементальное обновление |
| Query engine (F3) | dual-level retrieval (LightRAG), PathRAG (без map-reduce), PPR (без LLM на извлечении), multi-hop, grounding, детекция противоречий, LLM-as-judge |
| MCP-сервер (F6) | context compiler: tools/resources/prompts, сверка со спецификациями, предзавершающий гейт |
...
Заглушка GitLab, эмулятор LLM (детерминированные golden-ответы), тестовые хранилища графа/векторов, тестовая шина сигналов, webhook/MR-приёмник, мониторинг/аудит stub.

### 9.3. Минимальный набор по типу пути

| Тип E2E API-пути | Минимально поднять |
|---|---|
| MR/webhook → граф (F1) | заглушка GitLab + пайплайн индексации + тестовое хранилище графа |
| Запрос → ответ (F3) | онтологический слой + query engine + эмулятор LLM + хранилища графа/векторов |
| Изменение → влияние (F4) | ядро (анализ влияния) + хранилища + тестовая шина сигналов |
| Сигнал → знания (F5) | тестовая шина сигналов + пайплайн обработки сигналов |
| Агент → MCP-контекст (F6) | MCP-сервер + query engine + эмулятор LLM + хранилища |
| Обращение к LLM (F7) | эмулятор LLM + фильтр утечек/аудит stub |
...
- шина сигналов: формат сигнала, повторная доставка, идемпотентность.

### Шаг 3. Выполнить сквозные E2E API-сценарии

Прогнать наборы сценариев с уровнем `E2E API` из §12 / `qa/test-plan.md` (F1-01…F7-03, K1/K4/K5/K17).
...
Прогнать security-gap и доменные негативы (F7, §11):
...
- LLM-запрос с утечкой ПДн блокируется фильтром контура (F7.2) и фиксируется как SecurityEventEmitted;
- обращение к LLM вне корпоративного канала: целевая доля — 0 вне контура (F7.2);
- повторная доставка сигнала (F5) не дублирует сущности/утверждения.
...
| Пайплайны индексации (F1) | DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed; PDF-контур | статусы индексации, идемпотентность повторных событий |
| Query engine (F3) | retrieval (LightRAG/PathRAG/PPR), multi-hop, grounding | состав источников, QueryAnswered |
| Онтологический слой (F2) | загрузка DDD-модели, резолвинг | OntologyUpdated, конфликты терминов |
| Анализ влияния (F4) | граф связей, влияние изменения | InfluenceComputed, сигналы владельцу |
| MCP-сервер (F6) | context compiler | ContextCompiled, ReviewCompleted |
...
| Тестовая шина сигналов | логи | SignalReceived, повторная доставка, идемпотентность |
...
### 11.1. F1: индексация корпуса и граф знаний

E2E API-проверки:

- сквозной путь индексации: webhook/MR (GitLab) → DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed; корпус — markdown-спеки GitLab и PDF-книги (F1.1, F1.2);
- автоиндексация корпуса: новый/изменённый артефакт в GitLab → граф обновлён без ручных действий (F1.1);
- инкрементальное обновление: изменение части корпуса пересчитывает только затронутые подграфы, не трогая остальное (F1.3);
- PDF-контур: RAPTOR/StructRAG для книг — извлечение структуры и иерархии (F1.4);
- mutual indexing: сущности/связи/утверждения согласованы между графом и текстовым индексом (F1.5);
...
### 11.2. F2: онтологический слой

E2E API-проверки:

- загрузка/обновление DDD-модели: смена модели → OntologyUpdated, резолвинг начинает работать по новой модели (F2.1, F2.2);
- резолвинг запросов по онтологии: термины запроса сводятся к сущностям графа через единый язык (F2.1, F2.2);
- обновление сигналами кураторов: сигнал куратора → онтология обновлена → последующие запросы используют новую модель (F2.3);
- минимизация контекста: число токенов и LLM-вызовов на запрос не превышает целевого (F2.4, метрика «минимизация токенов/LLM-вызовов»).

Обязательные negative cases: невалидная DDD-модель (не проходит загрузку), конфликт терминов в модели (явная ошибка, а не тихий перевес), сигнал куратора с невалидными данными, запрос с термином вне онтологии.

### 11.3. F3: ответы

E2E API-проверки:

- сквозной путь ответа: запрос → QueryAnswered → ответ со ссылками на источники (grounding) (F3.1);
- dual-level retrieval (LightRAG): глобальный и локальный уровни поиска согласованы (F3.2);
- PathRAG: multi-hop по путям графа без map-reduce (F3.3);
- PPR: извлечение кандидатов без LLM (детерминизм) (F3.3);
- детекция противоречий: конфликтующие утверждения в корпусе → ConflictDetected, ответ помечается (F3.4);
- LLM-as-judge: оценка качества ответа (точность ≥ 70 %) на полном пути (F3.5);
- глобальные вопросы: ответ агрегирует по community summaries (C0–C1) (F3.2).
...
### 11.4. F4: трассируемость и анализ влияния

E2E API-проверки:

- граф связей артефактов: каждый артефакт связан с сущностями/утверждениями (F4.1; метрика «доля артефактов со связями»);
- влияние изменения: изменение артефакта → InfluenceComputed → список затронутых артефактов/запросов (F4.1, F4.2);
- сверка «результат ↔ источник»: каждый факт ответа сходится к источнику (F4.2; K5);
- граф до клиентского результата: цепочка от источника до выданного ответа прослеживается (F4.3);
- сигнал владельцу: при затронутости → сигнал владельцу артефакта (F4.4);
...
### 11.5. F5: накопление знаний

E2E API-проверки:

- приём сигналов с полей: SignalReceived → структурирование → привязка к сценарию/требованию (F5.1);
- сигналы → требования/тесты: сигнал формирует артефакт для требований и тестов (F5.1);
- разбор инцидентов: инцидент → знание, доступное для будущих запросов (F5.2);
- метрика времени решения: время сигнал→фикс измеряется на полном пути (F5.3);
- обновление из прода/телеметрии: KnowledgeRefreshed при поступлении новых данных (F5.4);
- повторная доставка сигналов: дубликат сигнала не меняет знания дважды (идемпотентность).

Обязательные negative cases: невалидный сигнал, дубликат сигнала, обрыв шины сигналов (ретрай без потери), сигнал без привязки к сценарию.

### 11.6. F6: context compiler MCP

E2E API-проверки:

- компиляция контекста: запрос агента → ContextCompiled → точный контекст агенту (F6.1);
- сверка со спецификациями: скомпилированный контекст сверен со спеками корпуса → ReviewCompleted (F6.2);
- предзавершающий гейт: агент не получает контекст, если сверка не прошла (F6.3);
- QA-контекст: подготовка контекста для QA-сценариев (F6.4);
...
### 11.7. F7: безопасный контур LLM

E2E API-проверки:

- сквозной путь обращения к LLM: запрос → локальный GPU-контур → фильтрация утечек → аудит (F7.1, F7.2);
- фильтрация утечек: ПДн/секреты в промпте или ответе блокируются (F7.2);
- аудит: каждое обращение фиксируется как SecurityEventEmitted и доступно ИБ (F7.2);
- контроль затрат: лимиты токенов/вызовов на запрос и период (F7.3);
- лицензии: open-weight модели и open-source компоненты проходят SCA-контроль; защита ПДн — в рамках 152-ФЗ (F7.2);
- доля обращений через корпоративный канал: целевое 100 % (0 вне контура) (F7.2, метрика).
...
Полные описания — в `qa/test-plan.md` §7. Ниже свод идентификаторов с уровнем `E2E API`. Идентификаторы `F1-01`…`F7-03` — заготовки: финальные scenario ID формируются в `qa/test-plan.md` (уточняется на этапе проектирования).

| Функция (vision §2.1–2.2) | Идентификаторы сценариев | Источник трассировки |
|---|---|---|
| F1 Индексация корпуса и граф знаний | F1-01†…F1-05† | F1.1–F1.5 (vision.md §2.2) |
| F2 Онтологический слой | F2-01†…F2-04† | F2.1–F2.4 (vision.md §2.2) |
| F3 Ответы | F3-01†…F3-05† | F3.1–F3.5 (vision.md §2.2) |
| F4 Трассируемость и анализ влияния | F4-01†…F4-04† | F4.1–F4.4 (vision.md §2.2) |
| F5 Накопление знаний | F5-01†…F5-04† | F5.1–F5.4 (vision.md §2.2) |
| F6 Context compiler MCP | F6-01†…F6-04† | F6.1–F6.4 (vision.md §2.2) |
| F7 Безопасный контур LLM | F7-01†…F7-03† | F7.1–F7.3 (vision.md §2.2) |
| Контрактные кейсы (эталонные) | K1, K4, K5, K17 | эталонные кейсы: контрактные тесты до реализации |
| Метрики / NFR-прокси | точность RAG ≥ 70 %, время ответа (медиана, p90), число противоречий, доля артефактов со связями, время сигнал→фикс, доля обращений к LLM через корпоративный канал (0 вне контура), минимизация токенов/LLM-вызовов | не функциональные сценарии: vision.md §1.3 |
...
| Онтология (DDD-модель) | валидная/невалидная модель, конфликты терминов, версии модели |
...
| Сигналы (F5) | валидные/невалидные, дубликаты, повторная доставка |
...
```text
Scenario F3-01: passed, потому что query engine вернул ответ.
```
...
- Онтологический слой выполнил резолвинг терминов по DDD-модели.
- Query engine выполнил dual-level retrieval (LightRAG) + PPR; PathRAG — без map-reduce.
...
1. **Функция (vision.md §2.2)** — ожидаемый результат по критерию F1.1–F7.3;
...
- [ ] Есть scenario ID (`F1-01`…`F7-03`, `K1/K4/K5/K17`, `C-*`) с уровнем E2E API.
...
1. `../vision.md` §2.1–2.2 — функции F1.1–F7.3 по пути: основной источник сценария и его критерии;
2. контракты всех сегментов: HTTP API, MCP (JSON-RPC 2.0), GitLab API/webhook/MR, LLM-контур (OpenAI-совместимый endpoint), хранилища, шина сигналов;
...
- **R1 (security/152-ФЗ):** часть проверок фиксирует целевое поведение после усиления (фильтрация утечек, аудит F7) — сценарии исполняются как expected fail/known gap до реализации.
...
- **R4 (to be):** часть функций — целевая модель (PDF-контур RAPTOR/StructRAG, инкрементальное обновление, предзавершающий гейт F6), реализация может отсутствовать.
...
- `../vision.md` — видение: функции F1–F7 (§2.1–2.2), события (§2.4), цели (§1.3), внешние зависимости (§2.8).
- `../glossary.md` — единый язык терминов (F2.2).
...
- `../src/README.md` — модули и компоненты (ядро, оркестрация пайплайнов, query engine, MCP-сервер, интеграции, CPU-компоненты).
