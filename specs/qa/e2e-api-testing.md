...
// F2.1: резолвинг запроса по модели предметной области — контекст.
...
| Онтологический слой (F2) | загрузка модели предметной области, резолвинг | OntologyUpdated, конфликты терминов |
...
- загрузка/обновление модели предметной области: смена модели → OntologyUpdated, резолвинг начинает работать по новой модели (F2.1, F2.2);
- резолвинг запросов по онтологии: термины запроса сводятся к сущностям графа через канонические определения (F2.1, F2.2);
- обновление модели из репозитория: новая версия модели → онтология обновлена → последующие запросы используют новую модель (F2.3);
...
Обязательные negative cases: невалидная модель предметной области (не проходит загрузку), конфликт терминов в модели (явная ошибка, а не тихий перевес), невалидные данные сигнала, запрос с термином вне онтологии.
