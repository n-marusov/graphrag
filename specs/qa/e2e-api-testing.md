...
| Объект | Полные пути через ядро, пайплайны индексации (F2), онтологический слой (механизм F1/F3), query engine (F1), анализ влияния (F3), MCP-сервер (F4), контур LLM (NFR-SEC-1) и тестовый контур по контрактам HTTP API / MCP (JSON-RPC 2.0) / GitLab API / LLM-контур / хранилища |
...
| Мониторинг/аудит | SecurityEventEmitted от реального контура до приёмника (NFR-SEC-1) |
...
|---|---|---|---|
| HTTP API | клиент ↔ контур | HTTP-клиент (запрос/ответ, метрики, здоровье) → ядро/query engine → ответ или событие в тестовом контуре | F1-01†, F3-01† |
...
|---|---|---|
| MR → индексация → граф → ответ со ссылками (F2+F1) | GitLab webhook/MR → пайплайн индексации → хранилище графа → community summaries → query engine → ответ | события DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed; ответ содержит ссылки на источники (grounding) |
...
| Обращение к LLM → фильтрация → аудит (NFR-SEC-1) | query engine/пайплайн → контур LLM → фильтр утечек → аудит-журнал → SecurityEventEmitted | утечки блокируются, аудит полный, 0 обращений вне корпоративного канала |
...
| Query engine (F1) | dual-level retrieval (LightRAG), PathRAG (без map-reduce), PPR (без LLM на извлечении), multi-hop, grounding, детекция противоречий, LLM-as-judge |
...
| Запрос → ответ (F1) | онтологический слой + query engine + эмулятор LLM + хранилища графа/векторов |
| Изменение → влияние (F3) | ядро (анализ влияния) + хранилища |
| Агент → MCP-контекст (F4) | MCP-сервер + query engine + эмулятор LLM + хранилища |
| Обращение к LLM (NFR-SEC-1) | эмулятор LLM + фильтр утечек/аудит stub |
...
Прогнать security-gap и доменные негативы (NFR-SEC-1, §11):
...
- LLM-запрос с утечкой ПДн блокируется фильтром контура (NFR-SEC-1) и фиксируется как SecurityEventEmitted;
- обращение к LLM вне корпоративного канала: целевая доля — 0 вне контура (NFR-SEC-1).
...
| Query engine (F1) | retrieval (LightRAG/PathRAG/PPR), multi-hop, grounding | состав источников, QueryAnswered |
...
- минимизация контекста: число токенов и LLM-вызовов на запрос не превышает целевого (NFR-PERF-1, метрика «минимизация токенов/LLM-вызовов»).
...
- LLM-as-judge: оценка качества ответа (точность ≥ 70 %) на полном пути (NFR-ACC-1);
...
### 11.7. Сквозное требование: безопасный контур LLM (NFR-SEC-1)

E2E API-проверки:

- сквозной путь обращения к LLM: запрос → локальный GPU-контур → фильтрация утечек → аудит (NFR-SEC-1);
- фильтрация утечек: ПДн/секреты в промпте или ответе блокируются (NFR-SEC-1);
- аудит: каждое обращение фиксируется как SecurityEventEmitted и доступно ИБ (NFR-SEC-1);
- контроль затрат: лимиты токенов/вызовов на запрос и период (NFR-PERF-1);
- лицензии: open-weight модели и open-source компоненты проходят SCA-контроль; защита ПДн — в рамках 152-ФЗ (NFR-SEC-1);
- доля обращений через корпоративный канал: целевое 100 % (0 вне контура) (NFR-SEC-1, метрика).
...
| NFR-SEC-1 — безопасный контур LLM (сквозное требование) | SEC-01†…SEC-08† | NFR-SEC-1 (vision.md §2.5) |
...
- **R1 (security/152-ФЗ):** часть проверок фиксирует целевое поведение после усиления (фильтрация утечек, аудит NFR-SEC-1) — сценарии исполняются как expected fail/known gap до реализации.
