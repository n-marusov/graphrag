...
|---|---|
| P0 | Безопасный контур LLM (фильтрация утечек, аудит — NFR-SEC-1), ответы с grounding и ссылками на источники (F1), целостность графа и инкрементальное обновление (F2), каскады событий предметной области, контрактные кейсы K1/K4/K5/K17 |
...
- если проверяется security-gap без оформленного требования (например, NFR-SEC-1 — контур LLM), нужно указать ID security-gap и ближайшее требование; отсутствие требования — вопрос к vision.md.
...
| LLM-inference endpoint (OpenAI-совместимый) | (TBD) | query engine/извлечение ↔ GPU-контур | F2, F1, NFR-SEC-1 |
| Хранилище графа и векторного индекса | выбор открыт — ADR-IMPL.DATA.graph-storage | модули ↔ хранилище | F1, F2 |
| Мониторинг и аудит | (TBD) | система → ИБ/мониторинг | NFR-SEC-1 (SecurityEventEmitted) |
...
| `SecurityEventEmitted → ИБ` | событие безопасности контура LLM доставляется в аудит/ИБ с корректной подписью и correlation ID (NFR-SEC-1) |
...
| LLM-контур | эмулятор LLM | детерминированные golden-ответы на OpenAI-совместимом endpoint, отказ/таймаут/дрейф ответов | F2 (извлечение), F1 (ответы), NFR-SEC-1 |
...
| Мониторинг/аудит | аудит-заглушка | приём SecurityEventEmitted и подтверждение доставки | NFR-SEC-1 |
...
| Аудит-заглушка | приём SecurityEventEmitted и подтверждение (NFR-SEC-1) |
...
| Контур LLM (NFR-SEC-1) | query engine + эмулятор LLM (или реальный GPU-контур) + аудит-заглушка |
...
- запросы к LLM идут только через корпоративный канал (локальный GPU-контур); 0 обращений вне контура (метрика NFR-SEC-1);
- фильтрация утечек ПДн/чувствительных данных при обращении к LLM (NFR-SEC-1), требования 152-ФЗ;
...
- минимизация контекста (NFR-PERF-1): в ответ и контекст попадает минимум необходимых сущностей.
...
- фильтрация утечек ПДн/чувствительных данных (NFR-SEC-1), требования 152-ФЗ;
- аудит обращений: `SecurityEventEmitted` доставляется в ИБ;
- лицензии open-weight моделей и open-source зависимостей (SCA);
- контроль затрат: минимизация токенов/LLM-вызовов (NFR-PERF-1).
...
| NFR-SEC-1 — безопасный контур LLM (сквозное требование) | (TBD) |
...
- **R4 (to be):** часть функций F1–F4 (например, mutual indexing (F2), LLM-as-judge (NFR-ACC-1), предзавершающий гейт F4 — Фаза 2/3) — целевая модель; реализация может отсутствовать.
