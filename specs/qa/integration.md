...
2. **Спецификации контрактов** — OpenAPI 3.1 (HTTP API), JSON Schema 2020-12 (payload), MCP JSON-RPC (tools/resources/prompts), Go-интерфейсы пакетов (внутренние контракты). Формируются на этапе проектирования и фиксируются в отдельном контрактном слое перед реализацией.
3. **Политика версионирования контрактов** — SemVer, additive-only, breaking changes, депрекации. Для текущего этапа: additive changes без breaking changes; breaking changes только через явную миграцию и version bump.
4. **Требования** — функции F1–F4 (vision.md §2.1–2.2), цели G1–G7 (§1.3), метрики; каталоги требований (функциональных, бизнес-правил, use cases, нефункциональных) уже существуют и являются источником трассировки.
5. **`qa/test-plan.md`** — доменные сценарии по функциям F1–F4; каталог scenario ID ведётся в отдельном реестре сценариев и может расширяться без изменения существующих ID.
6. **C4-модель** — актуализируется по мере изменения архитектуры; референс: `.ai-factory/references/mermaid-c4-diagrams.md`.
...
5. Найти доменный сценарий в `qa/test-plan.md` (§7) или создать новый ID при расширении плана (каталог scenario ID ведётся в `qa/test-plan.md` и дополняется без изменения существующих ID).
...
Сценарий QA: <scenario-id> (каталог ведётся в `qa/test-plan.md`)
...
- если сценарий основан на `qa/test-plan.md`, дополнительно указывать ID сценария в имени теста, subtest name, metadata или отчёте: например, `K1`, `K4`, `K5`, `K17` (эталонные контрактные кейсы); остальные scenario ID — в каталоге `qa/test-plan.md`;
...
|---|---|---|---|
| Интерфейсы пакетов ядра | ядро ↔ оркестрация пайплайнов | `../src/README.md`; контрактные спецификации хранятся в `qa/test-plan.md` и соответствующих `specs/`-артефактах | F2 (K1) |
| EventBus (паттерн) | модуль ↔ модуль | `../src/README.md`; события и каскады описаны в `vision.md §2.4` | каскады событий (vision.md §2.4) |
| Хранилище графа/векторов | модули ↔ хранилище | выбор открыт — ADR-IMPL.DATA.graph-storage | F1, F2 |
| Контракты движка запросов | движок запросов ↔ MCP-сервер / HTTP API | `../src/README.md`; контрактные спецификации хранятся в `qa/test-plan.md` и `specs/` | F1 (K1, K4, K5) |
| MCP-контракт (JSON-RPC: tools/resources/prompts) | MCP-сервер ↔ ИИ-агенты | `../src/README.md`; контрактные спецификации хранятся в `qa/test-plan.md` и `specs/` | F4 (K17) |
...
|---|---|---|---|
| GitLab API/webhook/MR | `../vision.md` §2.8; контрактные спецификации хранятся в `qa/test-plan.md` и `specs/` | GitLab → оркестрация пайплайнов | F2 (индексация) |
| HTTP API (запросы, метрики) | `qa/test-plan.md` + `specs/` | клиенты ↔ движок запросов | F1, G1 |
| MCP (JSON-RPC: tools/resources/prompts) | `qa/test-plan.md` + `specs/` | ИИ-агенты ↔ MCP-сервер | F4 |
| LLM-inference endpoint (OpenAI-совместимый) | `qa/test-plan.md` + `specs/` | движок запросов/извлечение ↔ GPU-контур | F2, F1, REQ-NFR-security.compliance.llm-contour |
| Хранилище графа и векторного индекса | выбор открыт — ADR-IMPL.DATA.graph-storage | модули ↔ хранилище | F1, F2 |
| Мониторинг и аудит | `qa/test-plan.md` + `specs/` | система → ИБ/мониторинг | REQ-NFR-security.compliance.llm-contour (SecurityEventEmitted) |
...
Для каждого проверяемого контракта выполнить conformance-сценарии из `qa/test-plan.md` (каталог scenario ID ведётся в `qa/test-plan.md`):
...
Прогнать доменные сценарии по функциям F1–F4 с уровнем `integration` из `qa/test-plan.md` (каталог scenario ID ведётся в `qa/test-plan.md`; на этапе видения — сценарии по событиям vision.md §2.4).
...
|---|---|
| F2 — индексация источников знаний | сценарии каталога `qa/test-plan.md` |
| Онтологический слой (механизм F1/F3) | сценарии каталога `qa/test-plan.md` |
| F1 — генерация проверяемых ответов | K1 (ответ с grounding); остальные сценарии каталога `qa/test-plan.md` |
...
| REQ-NFR-security.compliance.llm-contour — безопасный контур LLM (сквозное требование) | сценарии каталога `qa/test-plan.md` |
| Контракты / security-gaps | сценарии каталога `qa/test-plan.md` |
...
Если integration-тесты разнесены по модулям, запускать затронутый модуль по структуре `src/`:
...
- [ ] указан scenario ID из `qa/test-plan.md` или создан новый ID, внесённый в каталог `qa/test-plan.md`;
...
- [ ] Есть scenario ID из `qa/test-plan.md` или эталонный кейс K1/K4/K5/K17.
...
```text
Scenario IDs:
- <ID> (каталог `qa/test-plan.md`): <что проверяет>
...
- **R6 (открытые вопросы):** открытые вопросы фиксируются только в `specs/open-questions.md`; на текущем этапе их нет, поэтому влияние на контракты не ожидается.
...
- `qa/test-plan.md` — тест-план по функциям F1–F4 (каталог scenario ID ведётся в `qa/test-plan.md`).
