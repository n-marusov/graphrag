...
Интеграционный тест отвечает на вопрос: «работает ли согласованное поведение системы на границе компонентов так, как требуют требования (функции F1–F7, vision.md §2.1–2.2) и контракты модулей?». Он не заменяет unit-тесты и не должен превращаться в полный E2E без возможности локализовать дефект.
...
|---|---|
| P0 | Безопасный контур LLM (фильтрация утечек, аудит — F7), ответы с grounding и ссылками на источники (F3), целостность графа и инкрементальное обновление (F1), каскады событий предметной области, контрактные кейсы K1/K4/K5/K17 |
| P1 | Основные доменные потоки F1–F7, retry/timeout/idempotency, стыки с GitLab, HTTP API, MCP (JSON-RPC) и LLM-контуром, каскады событий, persistence при рестарте |
...
| Контракты с внешними системами | GitLab API/webhook/MR (индексация корпуса), HTTP API (запросы, метрики), MCP (JSON-RPC: tools/resources/prompts), LLM-inference endpoint (OpenAI-совместимый), хранилище графа и векторного индекса (выбор открыт — ADR-IMPL.DATA.graph-storage), импорт сигналов (F5), мониторинг/аудит (SecurityEventEmitted) |
| Каскады событий | DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed; OntologyUpdated → пересчёт резолвинга; InfluenceComputed → сигнал владельцу; ConflictDetected → сигнал на изменение источника; SignalReceived → KnowledgeRefreshed; ContextCompiled → ReviewCompleted; SecurityEventEmitted → ИБ |
...
1. **`../vision.md`** — цели G1–G7 (§1.3), функции F1–F7 (§2.1–2.2), события предметной области (§2.4), внешние зависимости (§2.8), метрики.
...
4. **Требования** — функции F1–F7 (vision.md §2.1–2.2), цели G1–G7 (§1.3), метрики; каталоги требований (функциональных, бизнес-правил, use cases, нефункциональных) планируются (vision.md §2.2).
5. **`qa/test-plan.md`** — доменные сценарии по функциям F1–F7; каталог scenario ID — (TBD), эталонные контрактные кейсы K1/K4/K5/K17.
...
1. Найти требование из vision.md (функция F1–F7, §2.1–2.2), цель (G1–G7, §1.3) или security-gap, которые реализуются изменением. Точные trace-коды появятся с каталогом требований (планируется, vision.md §2.2); до этого — ссылаться на `F<номер>.<подпункт>`.
...
8. Привязать сценарий к требованию (F1–F7), цели (G1–G7) и контракту в описании теста или отчёте.
...
```go
// F1.3: приём документа (MR/webhook → DocumentIngested) приводит к обновлению затронутого подграфа.
// F4.4: изменение артефакта формирует сигнал владельцу (InfluenceComputed).
...
- ID должен существовать в vision.md (F1–F7) или в каталоге требований (планируется, vision.md §2.2);
...
- если проверяется security-gap без оформленного требования (например, F7 — контур LLM), нужно указать ID security-gap и ближайшее требование; отсутствие требования — вопрос к vision.md.

Для ручных или полуавтоматических прогонов трассировка фиксируется в отчёте:

```text
Scenario: K1
Trace: F3.1 (ссылки на источники), метрика «точность ответов RAG ≥ 70%»
...
|---|---|---|---|
| Интерфейсы пакетов ядра | ядро ↔ оркестрация пайплайнов | `../src/README.md`; спецификации — (TBD) | F1 (K1) |
| EventBus (паттерн) | модуль ↔ модуль | — | каскады событий (vision.md §2.4) |
| Хранилище графа/векторов | модули ↔ хранилище | выбор открыт — ADR-IMPL.DATA.graph-storage | F1, F3 |
| Контракты query engine | query engine ↔ MCP-сервер / HTTP API | `../src/README.md`; спецификации — (TBD) | F3 (K1, K4, K5) |
| MCP-контракт (JSON-RPC: tools/resources/prompts) | MCP-сервер ↔ ИИ-агенты | `../src/README.md`; спецификации — (TBD) | F6 (K17) |
...
|---|---|---|---|
| GitLab API/webhook/MR | `../vision.md` §2.8; спецификации — (TBD) | GitLab → оркестрация пайплайнов | F1 (индексация) |
| HTTP API (запросы, метрики) | (TBD) | клиенты ↔ query engine | F3, G1 |
| MCP (JSON-RPC: tools/resources/prompts) | (TBD) | ИИ-агенты ↔ MCP-сервер | F6 |
| LLM-inference endpoint (OpenAI-совместимый) | (TBD) | query engine/извлечение ↔ GPU-контур | F1, F3, F7 |
| Хранилище графа и векторного индекса | выбор открыт — ADR-IMPL.DATA.graph-storage | модули ↔ хранилище | F1, F3 |
| Импорт сигналов (F5) | (TBD) | поля/телеметрия → ядро | F5 |
| Мониторинг и аудит | (TBD) | система → ИБ/мониторинг | F7 (SecurityEventEmitted) |
...
| `OntologyUpdated → пересчёт резолвинга` | обновление DDD-модели пересчитывает резолвинг запросов по терминам (F2) |
| `InfluenceComputed → сигнал владельцу` | анализ влияния изменения доставляет сигнал владельцу затронутого артефакта (F4) |
| `ConflictDetected → сигнал на изменение источника` | детекция противоречия между ответом и источником формирует сигнал (F3.4/F4.2) |
| `SignalReceived → KnowledgeRefreshed` | структурированный сигнал с поля обновляет знания (F5) |
| `ContextCompiled → ReviewCompleted` | скомпилированный контекст для агента проходит сверку со спецификациями (F6) |
| `SecurityEventEmitted → ИБ` | событие безопасности контура LLM доставляется в аудит/ИБ с корректной подписью и correlation ID (F7) |
...
|---|---|---|---|
| GitLab (API/webhook/MR) | заглушка GitLab | события MR/webhook, файлы корпуса (markdown-спеки, PDF-книги), отказ/retry | F1 (индексация) |
| LLM-контур | эмулятор LLM | детерминированные golden-ответы на OpenAI-совместимом endpoint, отказ/таймаут/дрейф ответов | F1 (извлечение), F3 (ответы), F7 |
| Хранилище графа/векторов | тестовое хранилище | граф сущностей/связей/утверждений, векторный индекс, инкрементальные обновления | F1, F3 |
| Шина сигналов | тестовая шина сигналов | приём структурированных сигналов с полей (F5), дубликаты | F5 |
| Мониторинг/аудит | аудит-заглушка | приём SecurityEventEmitted и подтверждение доставки | F7 |
| MCP | тестовый MCP-клиент | tools/resources/prompts, сверка контекста (ContextCompiled → ReviewCompleted) | F6 |
...
| Оркестрация пайплайнов (F1) | индексация корпуса: DocumentIngested → EntityExtracted → GraphUpdated → CommunityRecomputed; обработка PDF (RAPTOR/StructRAG); mutual indexing | GitLab API/webhook/MR, CPU-компоненты, хранилище, LLM-контур |
| Query engine (F3) | запросы: dual-level retrieval (LightRAG), PathRAG (без map-reduce), PPR, multi-hop, grounding (QueryAnswered), детекция противоречий (ConflictDetected), LLM-as-judge | LLM-inference endpoint, хранилище графа/векторов, HTTP API |
| MCP-сервер (F6) | компиляция контекста агенту (ContextCompiled), сверка со спецификациями (ReviewCompleted), предзавершающий гейт, QA-контекст | MCP (JSON-RPC: tools/resources/prompts), query engine |
...
| Тестовая шина сигналов | импорт структурированных сигналов (F5) |
| Аудит-заглушка | приём SecurityEventEmitted и подтверждение (F7) |
| MCP-клиент (агент-заглушка) | потребление контекста, сверка со спецификациями (F6) |
...
| MCP-контракт (F6) | MCP-сервер + query engine + эмулятор LLM |
| Обработка PDF (RAPTOR/StructRAG) | оркестрация + CPU-компоненты + эмулятор LLM |
| Контур LLM (F7) | query engine + эмулятор LLM (или реальный GPU-контур) + аудит-заглушка |
| Импорт сигналов (F5) | ядро + тестовая шина сигналов + хранилище |
...
Прогнать доменные сценарии по функциям F1–F7 с уровнем `integration` из `qa/test-plan.md` (каталог scenario ID — (TBD); на этапе видения — сценарии по событиям vision.md §2.4).
...
- запросы к LLM идут только через корпоративный канал (локальный GPU-контур); 0 обращений вне контура (метрика F7);
- фильтрация утечек ПДн/чувствительных данных при обращении к LLM (F7.2), требования 152-ФЗ;
...
- trace на требование (F1–F7, vision.md), цель (G1–G7) или security-gap;
...
## 11. Специфика по функциям проекта (F1–F7)
...
- расхождение извлечённых сущностей с DDD-моделью (сигнал на F2);
...
- загрузка машинно-читаемой DDD-модели и резолвинг запросов по терминам (F2.1);
- единый язык: разные формулировки запроса резолвятся в одни и те же сущности (F2.2);
- сигналы кураторов о расхождении терминов → `OntologyUpdated` → пересчёт резолвинга (F2.3);
- минимизация контекста (F2.4): в ответ и контекст попадает минимум необходимых сущностей.

Negative/boundary:

- неизвестный термин вне DDD-модели;
- конфликт синонимов/омонимов;
- пустая или частично загруженная модель;
- обновление модели на середине запроса.

### 11.3. F3 — ответы с grounding

Проверять:

- сквозной путь запроса: `QueryAnswered` со ссылками на источники (grounding/provenance, F3.1);
- dual-level retrieval (LightRAG), PathRAG (без map-reduce), PPR (без LLM на извлечении);
- глобальные вопросы (F3.2) и multi-hop запросы (F3.3);
- детекция противоречий: `ConflictDetected` при расхождении ответа и источника (F3.4);
- LLM-as-judge (F3.5);
...
- граф связей артефактов (F4.1): список затронутых при изменении;
- `InfluenceComputed` — сигнал владельцу затронутого артефакта (F4.4);
- сверка «результат ↔ источник» (F4.2): каждый факт ответа прослеживается до источника;
- граф от артефакта до клиентского результата (F4.3);
...
- приём структурированных сигналов с полей (`SignalReceived`);
- сигнал → требование/тест (F5.1), разбор инцидентов (F5.2);
- обновление знаний из прода/телеметрии (`KnowledgeRefreshed`, F5.4);
- метрика «время сигнал → фикс» (цель G4).

Negative/boundary:

- невалидный или дублирующий сигнал;
- сигнал без владельца (owner);
- конфликт сигнала с текущим знанием.
...
- фильтрация утечек ПДн/чувствительных данных (F7.2), требования 152-ФЗ;
- аудит обращений: `SecurityEventEmitted` доставляется в ИБ;
- лицензии open-weight моделей и open-source зависимостей (SCA);
- контроль затрат: минимизация токенов/LLM-вызовов (F7.3).
...
|---|---|
| F1 — индексация корпуса и граф знаний | (TBD) |
| F2 — онтологический слой | (TBD) |
| F3 — ответы | K1 (ответ с grounding); остальные — (TBD) |
| F4 — трассируемость и анализ влияния | K4 (анализ влияния), K5 (сверка «результат ↔ источник») |
| F5 — накопление знаний | (TBD) |
| F6 — context compiler (MCP) | K17 (context compiler) |
| F7 — безопасный контур LLM | (TBD) |
...
| DDD-модель | валидная, неполная, конфликтующая (синонимы/омонимы), устаревшая |
| Golden-ответы эмулятора LLM | детерминированные ответы на фиксированные запросы, отказы/таймауты/дрейф ответов |
| Запросы | глобальные, multi-hop, вне корпуса, противоречивые, с терминами вне DDD-модели |
| Сигналы (F5) | структурированные сигналы с полей, дубликаты, невалидные, сигналы без владельца |
...
2. `../vision.md` — функции F1–F7, цели G1–G7, метрики;
...
1. **Проверяемые требования** — сценарий связан с требованием (F1–F7), целью (G1–G7) или security-gap.
...
```text
Scenario: <ID>
Trace: <F1–F7/цель G1–G7/security-gap>
...
4. `FG-TRACE` — требование (F1–F7, vision.md) ↔ тесты/сценарии.
...
8. `FG-NFR` — метрики: точность ответов RAG ≥ 70%, время ответа (медиана/p90), число противоречий, доля артефактов со связями, время «сигнал → фикс», доля обращений к LLM через корпоративный канал (0 вне контура), минимизация токенов/LLM-вызовов.
...
- [ ] есть привязка к требованию (F1–F7, vision.md), цели (G1–G7) или security-gap;
...
- [ ] Есть trace на требование (F1–F7, vision.md) или метрику в комментарии/metadata/отчёте.
...
1. `../vision.md` — функции F1–F7 (§2.1–2.2), цели G1–G7 (§1.3), события (§2.4), метрики;
2. спецификацию нужного контракта (при наличии; реестр планируется — vision.md §2.2);
3. `qa/test-plan.md` по scenario ID;
4. каталог требований по trace ID (планируется, vision.md §2.2) или F1–F7 из vision.md;
...
- [ ] есть trace на требование (F1–F7)/цель/security-gap;
...
- Не принимать integration-сценарий без trace на требование (F1–F7, vision.md), цель или security-gap.
...
- **R4 (to be):** часть функций F1–F7 (например, mutual indexing, LLM-as-judge, предзавершающий гейт F6.3) — целевая модель; реализация может отсутствовать.
- **R5 (каталоги):** каталоги требований и реестр контрактов планируются (vision.md §2.2); до их появления трассировка опирается на F1–F7, цели G1–G7 и метрики.
...
- `../vision.md` — цели G1–G7, функции F1–F7, события предметной области (§2.4), внешние зависимости (§2.8), метрики.
- `../glossary.md` — глоссарий терминов.
- `../adr/README.md` — реестр ADR (в т.ч. ADR-IMPL.STACK.go-single-language-adoption, ADR-IMPL.DATA.graph-storage).
- `../src/README.md` — описание модулей: ядро, оркестрация пайплайнов (F1), query engine (F3), MCP-сервер (F6), интеграции, CPU-компоненты.
...
- `qa/test-plan.md` — тест-план по функциям F1–F7 (каталог scenario ID — (TBD)).
