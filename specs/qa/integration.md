...
| `OntologyUpdated → пересчёт резолвинга` | обновление модели предметной области пересчитывает резолвинг запросов по терминам (F2) |
...
- расхождение извлечённых сущностей с моделью предметной области (сигнал на F2);
...
- загрузка машинно-читаемой модели предметной области и резолвинг запросов по терминам (F2.1);
- канонические определения: разные формулировки запроса резолвятся в одни и те же сущности (F2.2);
- обновление модели из репозитория → `OntologyUpdated` → пересчёт резолвинга (F2.3);
