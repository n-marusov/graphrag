...
| **Контур тестирования** | Модули GraphRAG: ядро, оркестрация пайплайнов (F2), движок запросов (F1), MCP-сервер (F4), интеграции (GitLab, GPU-контур), CPU-компоненты (Leiden, HNSW, токенизатор). Внешние зависимости — GitLab, локальный GPU-контур LLM (сквозное требование REQ-NFR-security.compliance.llm-contour), ИИ-агенты (MCP), мониторинг и аудит: тестируются **контракты** взаимодействия с ними, но не внутренняя функциональность внешних систем. Накопление знаний из внешних сигналов — вне рамок (vision.md §2.7 п.4–5) |
...
|---|---|
| **Verifiable** | Каждое FR имеет критерии приёмки; каждое NFR — метрику и целевое значение (каталог `nonfun-req/` — `REQ-NFR-*`, перенесены из vision.md §2.5). Без этого тест написать нельзя |
...
Схема — целевая иерархия: каталоги требований `use-cases/`, `user-stories/`, `business-rules/`, `nonfun-req/` ведутся; `fun-req/` — планируется (vision.md §2.2).
...
4. **Критичность определяет глубину.** Сценарии приоритетов P0–P2 покрываются на всех уровнях по убыванию: P0 — обязательно на каждом уровне, P2 — по решению. Критичные модули — максимальное покрытие: контур LLM (фильтрация утечек, аудит — REQ-NFR-security.compliance.llm-contour), онтологический слой (резолвинг — механизм F1/F3), извлечение знаний (F2), анализ влияния (детерминизм — F3), MCP-компилятор (F4), CPU-компоненты (токенизатор, HNSW, Leiden), инкрементальное обновление графа (F2).
5. **Негативные сценарии и границы обязательны.** Большинство дефектов — в обработке ошибок: некорректные запросы, повреждённые/несогласованные данные, недоступность LLM-контура/хранилища/GitLab, таймауты, идемпотентность, параллельные запросы, детекция противоречий (G3, событие `ConflictDetected` — vision.md §2.4).
...
9. **Безопасность тестируется отдельно и эшелонированно.** Статический анализ (SAST), сканирование зависимостей (SCA), поиск секретов, DAST, контроль целостности — в CI (целевое состояние); контролируемый контур LLM (G6, REQ-NFR-security.compliance.llm-contour): фильтрация утечек, аудит, контроль затрат/токенов. Целевые значения заданы в NFR и BR: доля обращений к LLM через корпоративный канал — 100%, обращений вне контура — 0, событие `SecurityEventEmitted` — немедленный алерт при > 0 событий, полнота audit-trail агентов — 100% шагов с обязательными полями. Проверяются тестами и отчётами, а не «отсутствием инцидентов».
...
13. **Среда и регуляторика — объект тестирования.** Совместимость со средой (развёртывание GPU-контура: драйверы, CUDA, рантайм моделей; протоколы, деградация при недоступности внешних систем) и регуляторные требования (152-ФЗ — система не оператор ПДн (vision §2.7 п.12); корпоративные правила ИБ; лицензии open-weight моделей и open-source — SCA) проверяются тестами наравне с функциями.
...
- Критичные модули (контур LLM — фильтрация утечек, аудит (REQ-NFR-security.compliance.llm-contour); онтологический слой — резолвинг (механизм F1/F3); извлечение знаний (F2); анализ влияния — детерминизм (F3); MCP-компилятор (F4); CPU-компоненты (токенизатор, HNSW, Leiden); инкрементальное обновление графа (F2)) покрываются приоритетно.
- Запуск с детектором гонок: `go test -race`.

**Критерии входа/выхода.** Вход: пакет реализован, интерфейсы выделены. Выход: тесты проходят с `-race`, покрытие критичных пакетов не ниже **80% line/branch** (порог целевого состояния CI, см. §8.2), отчёт покрытия публикуется в CI.
...
- Внутренние Go-контракты модулей (ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, интеграции, CPU-компоненты); контрактные тесты по эталонным кейсам K1, K4, K5, K17 (до реализации).
- Внешние контракты: GitLab API/webhook/MR (авторитетный источник корпуса — F2), HTTP API (запросы/метрики), MCP (JSON-RPC), локальный GPU-контур LLM (OpenAI-совместимый inference endpoint — REQ-NFR-security.compliance.llm-contour), хранилище графа и векторного индекса, мониторинг и аудит.
...
- Хранилище графа и векторного индекса (выбор открыт — ADR-IMPL.DATA.graph-storage), инкрементальное обновление графа (F2).
...
**Минимальный набор.** Критический путь P0 (не более 5–15 минут): индексация мини-корпуса; ответ на эталонный вопрос со ссылками на источники; CLI отвечает версией продукта; MCP-запрос; здоровье/метрики; аудит.
...
**База.** Пользовательские истории `user-stories/` (планируются — vision.md §2.2) в формате Gherkin (теги `@US-*`, `@UC-*`, `@P0–P2`, доменные теги, `@accessibility`): сценарии исполняются автоматизацией (BDD-конвейер Gherkin → шаги).

**Объект в проекте.** Веб-интерфейс системы знаний и дашборды администрирования (индексация, онтология, метрики, аудит контура LLM). Состав экранов фиксируется в UI-дизайн-артефактах Фазы 1 до реализации GUI-сценариев; это проектная детализация принятого web-канала, а не открытый вопрос требований. Сценарии доступности: screen reader, keyboard-only навигация — по **WCAG 2.1 AA** (уровень зафиксирован 2026-08-30, BR-constraint.web-app-browser-chat; состав сценариев — после фиксации UI).
...
|---|---|---|
| HTTP API | клиент ↔ движок запросов (F1) | ответы со ссылками на источники (F1.1), глобальные вопросы (F1.2), multi-hop (F1.2), метрики |
| MCP (JSON-RPC) | ИИ-агенты ↔ context compiler (F4) | точный контекст агенту (F4), сверка со спецификациями (F4, G4), предзавершающий гейт (Фаза 2/3), QA-контекст (F4) |
| GitLab API/webhook/MR | GitLab ↔ интеграции (F2) | автоиндексация корпуса (F2.1), инкрементальное обновление графа (F2) |
| LLM-inference (OpenAI-совместимый endpoint) | система ↔ локальный GPU-контур (REQ-NFR-security.compliance.llm-contour) | извлечение сущностей, ответы, детекция противоречий (G3), LLM-as-judge (REQ-NFR-api.compliance.rag-accuracy) |
| Хранилище графа и векторного индекса | система ↔ хранилище | чтение/запись графа, векторный поиск, инкрементальное обновление (F2); выбор хранилища открыт (ADR-IMPL.DATA.graph-storage) |
| Мониторинг/аудит | система ↔ администратор/ИБ | SecurityEventEmitted → ИБ, фильтрация утечек и аудит (REQ-NFR-security.compliance.llm-contour), контроль затрат/токенов (REQ-NFR-api.performance.token-minimization) |

**Правила.** Обязательны негативные сценарии и границы: некорректные запросы, повреждённые/несогласованные данные, недоступность LLM-контура/хранилища/GitLab, таймауты, повторная доставка (идемпотентность), параллельные запросы, детекция противоречий (G3). Проверяется соблюдение бизнес-правил `BR-*` ([business-rules/](../business-rules/README.md)).
...
- **Каждое требование FR/US/BR имеет тест минимум на одном уровне (100% требование-уровневое покрытие, 2026-08-30)**; P2 — по решению владельца.
...
|---|---|---|---|---|
| **1. Среда** | SCA (trivy), лицензии и политики open-source, SBOM; изоляция тест-среды от продуктивных данных | Совместимость: тестовый контур (GPU-стек, модели, браузеры/MCP-клиенты); регуляторика: 152-ФЗ (ПДн не обрабатываются — vision §2.7 п.12), лицензии моделей и open-source (SCA); деградация при недоступности внешних систем | Инкрементальное обновление графа и откаты; аудит-следы; SLA на реагирование при инцидентах | Тесты вывода контура из эксплуатации, ротации моделей, отзыва доступов; compliance при архивировании |
| **2. Эргономика** | Ревью UX/доступности требований; онбординг команды | E2E GUI: a11y (WCAG 2.1 AA, screen reader, keyboard-only), юзабилити, локализация (ru/en) | Читаемость и фильтрация логов, консоли/дашборды мониторинга | UX администратора при выводе контура из эксплуатации/миграции: CLI-инструменты, инструкции |
| **3. Функции** | Модульные и интеграционные тесты; критерии приёмки до кода | E2E GUI/API, smoke P0, нагрузочные тесты | Тесты инкрементального обновления графа (rolling, откат), мониторинга, каскадов событий | Тесты вывода контура из эксплуатации, ротации моделей, удаления данных/томов |
...
### 5.2. Официальные артефакты подтверждения статуса метрик

Переход ячейки матрицы в статус 🟢 подтверждается официальными артефактами (решение владельца продукта 2026-08-30):

| № | Артефакт | Что подтверждает | Владелец | Частота обновления |
|---|---|---|---|---|
| 1 | CI-отчёт `qa-benchmark` | Точность RAG и полнота контекста (метрики точности и F1) на эталонном наборе | Tech Lead / QA-инженер | При каждом PR, изменяющем ядро извлечения или запросов; еженедельная сводка по `main` |
| 2 | Дашборд золотых сигналов | Латентность p50/p95, доля ошибок, насыщенность, время публикации индекса | Администратор системы / DevOps | В реальном времени (streaming), визуализация за 24 часа |
| 3 | Отчёт `index-freshness` | Время MR → публикация снапшота (медиана/p90), доля ответов на актуальных версиях | Инженер по данным | Ежедневно (автоматический прогон на ночном корпусе) |
| 4 | Аудит-лог агентов (`agent-audit`) | Полнота следов (100% шагов с 10 полями), соблюдение бюджетов и лимитов | Security Engineer / Tech Lead | Ежедневно (проверка целостности по `trace-id`) |
| 5 | Отчёт `a11y-report` | Отсутствие Critical/Serious нарушений WCAG 2.1 AA | Frontend-разработчик | Еженедельно (CI на `main` / при релизе) |
| 6 | Журнал `SecurityEventEmitted` | Отсутствие инцидентов фильтрации утечек (0 событий) или их корректная обработка | Security Engineer | В реальном времени (алерт в SIEM/мониторинг) |

**Критерий «официальности»:**

- **Неопровержимость:** для метрик, где 🟢 означает «всё в порядке», официальным подтверждением является **отсутствие алертов** в корпоративной системе мониторинга в течение отчётного периода.
- **Источник для цифр:** для количественных метрик (проценты, время) эталоном считаются **CI-отчёты** с фиксированным временем прогона (`commit hash`) — они перезапускаемы и проверяемы; дашборды используются для текущей диагностики, а не для финальной отчётности.

### 5.3. Условия недействительности измерений и повторный прогон

Измерение недействительно и требует повторного прогона в следующих случаях (решение владельца продукта 2026-08-30):

| № | Условие | Тип аннулирования | Обоснование |
|---|---|---|---|
| 1 | Изменение состава корпуса (добавление/удаление/изменение документов в GitLab или внешнем хранилище) | Автоматическое | Метрики точности и полноты привязаны к снапшоту корпуса; изменение меняет эталонные ответы (`REQ-NFR-data.performance.index-freshness`) |
| 2 | Смена версии LLM-модели (локальной или внешней) | Автоматическое | Разные модели — разное качество извлечения и генерации; точность ≥ 70% подтверждается для каждой используемой модели |
| 3 | Смена стенда (тестовый → стейджинг → прод) | Ручное | Разная аппаратная конфигурация (CPU/GPU, память) влияет на латентность и пропускную способность; результаты не переносятся между стендами без повторного прогона |
| 4 | Флаки-тест (разброс > 5% по точности или > 10% по латентности между двумя прогонами на одном стенде с одним корпусом) | Автоматическое | Нестабильный результат требует расследования и перепрогона (`REQ-NFR-api.observability.golden-signals`) |
| 5 | Просроченный baseline (> 1 недели с момента последнего подтверждённого прогона) | Автоматическое | Метрики старше недели неактуальны, если корпус изменился (`REQ-NFR-data.performance.index-freshness`) |
| 6 | Изменение онтологической модели (TBox) | Автоматическое | Онтологический слой — основа резолвинга и минимизации токенов; изменение требует пересчёта всех метрик |
| 7 | Изменение методологии измерения (например, смена LLM-as-a-Judge на BERTScore) | Ручное | Разные метрики дают разные абсолютные значения; сравнение допустимо только внутри одной методологии (`REQ-NFR-api.compliance.rag-accuracy`) |

**Правило повторного прогона:**

- **Автоматический прогон** (условия 1, 2, 4, 5, 6): запускается автоматически в CI при обнаружении изменения; результат предыдущего измерения аннулируется до получения нового; статус метрики переводится в 🟡 (pending).
- **Ручной прогон** (условия 3, 7): инициируется владельцем метрики (Tech Lead / QA-инженер); проводится в течение 2 рабочих дней с момента принятия решения; результаты до перепрогона не считаются валидными для отчётности.
- **Флаки-исключение** (условие 4): проводится 3 последовательных прогона; валидным принимается медианный результат из трёх; разброс > 5% по точности — тест нестабилен, требуется инженерное расследование до следующего замера.

**Порядок действий при аннулировании:** фиксация причины в системе мониторинга/отчётности → уведомление владельца метрики (алерт в корпоративный чат/почту) → запуск повторного прогона (автоматически или вручную) → обновление статуса в дашборде/отчёте → архивация аннулированного измерения с пометкой причины (для аудита).
...
NFR задают измеримые границы качества (каталог `nonfun-req/` — `REQ-NFR-*`, перенесены из vision.md §2.5); каждый NFR проверяется тестом по своим критериям приёмки (метрика + целевое значение):

| Категория качества | Как проверяется | Примеры NFR (метрики) |
|---|---|---|
| Производительность | Нагрузочные тесты (HTTP API, MCP, индексация корпуса); замеры на стенде | Отзывчивость интерактивных запросов: p50 ≤ 5 с, p90 ≤ 10 с; деградация при фоновой переиндексации ≤ 1.5× от простоя; token budget: медиана ≤ 4 000 токенов, p90 ≤ 8 000; LLM-вызовы: медиана ≤ 2, абсолютный лимит ≤ 3 |
| Качество и полнота | Прогон golden-наборов «запрос → эталонный ответ → источники» на эмуляторе LLM; сверка результата с источником (G4, K5) | Точность ответов RAG ≥ 70%; полнота контекста и число противоречий замеряются на утверждённом эталонном наборе; результаты baseline Фазы 1.5 фиксируют текущий уровень, но не являются открытым вопросом |
| Доступность | Отказоустойчивость: деградация при недоступности LLM-контура/хранилища/GitLab, ретраи, восстановление/перестройка индекса и графа; перезапуск компонентов | Дневной бюджет простоя ≤ 15 мин на инстанс; RTO ≤ 15 мин на инцидент; RPO исходных артефактов = 0; restore-тест проходит K1/K4/K5/K17 |
| Безопасность | SAST (semgrep), SCA (trivy), поиск секретов, DAST; фильтрация утечек и аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | Доля обращений к LLM через корпоративный канал — 100%; обращений вне контура — 0; `SecurityEventEmitted` > 0 создаёт немедленный алерт; критичные уязвимости блокируют релиз |
| Совместимость | Прогоны на тестовом контуре: GPU-стек, модели, браузеры/MCP-клиенты | UI: Chrome/Edge/Firefox/Safari последние 2 мажорные версии; a11y — WCAG 2.1 AA без Critical/Serious нарушений; локализация ru/en; MCP — контрактные сценарии K1/K4/K5/K17 |
| Наблюдаемость | Проверка полноты и чистоты телеметрии и логов; аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | Golden signals: Error Ratio > 1% за 5 мин — warn; Queue Saturation > 80% за 5 мин — warn; latency p50 > 2 с или p95 > 5 с — warn; MR → публикация индекса p95 > 15 мин — warn; agent-audit — 100% шагов с обязательными полями |
...
- Проект на этапе видения: кодовой базы, тестов и CI реализации ещё нет (`src/` — только `README.md`). Методология QA фиксируется заранее и применяется к первому коду; это ограничение верификации, а не открытый вопрос требований.
...
- Пороги качества в CI зафиксированы в §8.2: покрытие критичных пакетов ≥ 80% line/branch, требование-уровневое покрытие FR/US/BR — 100%, smoke ≤ 15 мин; реестр открытых вопросов — `specs/open-questions.md`.

### 8.2. Целевое состояние

**Платформа CI/CD — GitLab CI** (решение 2026-08-30). Развёртывание тестового контура — **Docker Compose** (форма развёртывания MVP, `ADR-DES.INFRA.container-composition`). Конвейер контроля качества (полностью автоматический, на тестовом контуре):

```
lint (golangci-lint) → unit (go test -race + coverage)
  → build (Go-кросс-сборка; сборка фронта TS/Vite) → smoke (deploy на тестовом контуре, Docker Compose)
  → контрактные тесты (K1, K4, K5, K17) → integration (тестовый контур)
  → e2e API (HTTP/MCP) → e2e GUI (Playwright, браузер, в т.ч. a11y и локализация)
  → NFR (бенчмарки, RAGAS-оценка ответов — гейт FG-RAG-EVAL; DevSecOps-сканы: gosec, trivy, semgrep, поиск секретов)
  → релизные гейты
```

- Быстрый набор (lint + unit + build smoke) — на каждый push/MR; полный набор (все уровни) — ночной прогон и перед релизом.
- Пороги качества в CI: покрытие критичных пакетов (≥ 80% line/branch; требование-уровневое покрытие FR/US/BR — 100%), стабильность (допустимая доля «флаки»), время прогона (smoke ≤ 15 мин, SM-D-14).
...
| Владелец требований | Критерии приёмки, приоритизация, решение по P2-покрытию и открытым вопросам; на 2026-08-30 открытых вопросов по матрице качества нет, остаются только подтверждающие замеры |

## 12. Связанные артефакты

- [Видение продукта](../vision.md) — цели G1–G7, функции F1–F4 (§2.1–2.2), критерии успеха §1.4, события предметной области §2.4, внешние зависимости §2.8
...
- Каталоги требований: `use-cases/`, `user-stories/`, `business-rules/`, `nonfun-req/` — ведутся; `fun-req/` — планируется (vision.md §2.2); C4-модель планируется (`adr/README.md`; референс — `../../.ai-factory/references/mermaid-c4-diagrams.md`)
