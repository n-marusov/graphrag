...
**Объект в проекте.** Веб-интерфейс системы знаний и дашборды администрирования (индексация, онтология, метрики, аудит контура LLM); состав интерфейса не зафиксирован (уточняется на этапе проектирования). Сценарии доступности: screen reader, keyboard-only навигация — по **WCAG 2.1 AA** (уровень зафиксирован 2026-08-30, BR-constraint.web-app-browser-chat; состав сценариев — после фиксации UI).
...
- **Каждое требование FR/US/BR имеет тест минимум на одном уровне (100% требование-уровневое покрытие, Q3.8, 2026-08-30)**; P2 — по решению владельца.
...
| Качество и полнота | Прогон golden-наборов «запрос → эталонный ответ → источники» на эмуляторе LLM; сверка результата с источником (G4, K5) | Точность ответов RAG ≥ 70%; число противоречий; доля артефактов со связями в графе (целевые значения уточняются после замера) |
...
| Безопасность | SAST (semgrep), SCA (trivy), поиск секретов, DAST; фильтрация утечек и аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | Доля обращений к LLM через корпоративный канал; 0 вне контура |
| Совместимость | Прогоны на тестовом контуре: GPU-стек, модели, браузеры/MCP-клиенты | (TBD) |
| Наблюдаемость | Проверка полноты и чистоты телеметрии и логов; аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | Доля вопросов, закрытых без аналитика (G7); доля обращений к LLM через корпоративный канал (G6) (целевые значения уточняются после замера) |
...
**Платформа CI/CD — GitLab CI** (решение 2026-08-30, open-questions.md Q3.3). Развёртывание тестового контура — **Docker Compose** (форма развёртывания MVP, `ADR-DES.INFRA.container-composition`). Конвейер контроля качества (полностью автоматический, на тестовом контуре):
...
- Пороги качества в CI: покрытие критичных пакетов (≥ 80% line/branch; требование-уровневое покрытие FR/US/BR — 100%, Q3.8), стабильность (допустимая доля «флаки»), время прогона (smoke ≤ 15 мин, SM-D-14).
