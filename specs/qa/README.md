...
| **Контур тестирования** | Модули GraphRAG: ядро, оркестрация пайплайнов (F2), query engine (F1), MCP-сервер (F4), интеграции (GitLab, GPU-контур), CPU-компоненты (Leiden, HNSW, токенизатор). Внешние зависимости — GitLab, локальный GPU-контур LLM (сквозное требование NFR-SEC-1), ИИ-агенты (MCP), мониторинг и аудит: тестируются **контракты** взаимодействия с ними, но не внутренняя функциональность внешних систем. Накопление знаний из внешних сигналов — вне рамок (vision.md §2.7 п.4–5) |
...
- Критичные модули (контур LLM — фильтрация утечек, аудит (NFR-SEC-1); онтологический слой — резолвинг (механизм F1/F3); извлечение знаний (F2); анализ влияния — детерминизм (F3); MCP-компилятор (F4); CPU-компоненты (токенизатор, HNSW, Leiden); инкрементальное обновление графа (F2)) покрываются приоритетно.
...
- Внешние контракты: GitLab API/webhook/MR (авторитетный источник корпуса — F2), HTTP API (запросы/метрики), MCP (JSON-RPC), локальный GPU-контур LLM (OpenAI-совместимый inference endpoint — NFR-SEC-1), хранилище графа и векторного индекса, мониторинг и аудит.
...
| LLM-inference (OpenAI-совместимый endpoint) | система ↔ локальный GPU-контур (NFR-SEC-1) | извлечение сущностей, ответы, детекция противоречий (G3), LLM-as-judge (NFR-ACC-1) |
...
| Мониторинг/аудит | система ↔ администратор/ИБ | SecurityEventEmitted → ИБ, фильтрация утечек и аудит (NFR-SEC-1), контроль затрат/токенов (NFR-PERF-1) |

**Правила.** Обязательны негативные сценарии и границы: некорректные запросы, повреждённые/несогласованные данные, недоступность LLM-контура/хранилища/GitLab, таймауты, повторная доставка (идемпотентность), параллельные запросы, детекция противоречий (G3). Проверяется соблюдение бизнес-правил `BR-*` (каталог планируется — vision.md §2.2).
...
| Безопасность | SAST (semgrep), SCA (trivy), поиск секретов, DAST; фильтрация утечек и аудит контура LLM (NFR-SEC-1) | Доля обращений к LLM через корпоративный канал; 0 вне контура |
| Совместимость | Прогоны на тестовом контуре: GPU-стек, модели, браузеры/MCP-клиенты | (TBD) |
| Наблюдаемость | Проверка полноты и чистоты телеметрии и логов; аудит контура LLM (NFR-SEC-1) | Доля вопросов, закрытых без аналитика (G7); доля обращений к LLM через корпоративный канал (G6) (целевые значения уточняются после замера) |
