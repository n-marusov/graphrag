...
| Query engine | обработка запросов, ответы со ссылками на источники (F1, K1) |
...
| Контекстный режим MCP (F4, K17) | MCP-сервер + query engine + контур LLM (проверяется в E2E API; в GUI — статус/отображение) |
...
| Query engine | логи query engine | запросы, ответы, grounding (F1, K1) |
