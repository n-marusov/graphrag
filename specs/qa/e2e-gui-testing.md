...
### 11.3. Аудит контура LLM (REQ-NFR-security.compliance.llm-contour)
...
- доступ к интерфейсу/настройкам разрешён только авторизованным пользователям; попытка без корпоративной аутентификации (SSO Keycloak, `BR-constraint.sso-readonly-access`, `ADR-DES.SECURITY.sso-keycloak`) отклоняется с понятным сообщением; истёкший/невалидный OIDC-токен → перенаправление на вход SSO или понятная ошибка (входит в MVP);
...
Обязательные negative cases: неавторизованная попытка, невалидный/истёкший токен SSO, отсутствие записи аудита при выполненном действии — дефект.

### 11.4. Доступность (a11y)

E2E GUI-проверки (уровень **WCAG 2.1 AA**, `BR-constraint.web-app-browser-chat`, open-questions.md Q1.5; сценарии обязательны):
