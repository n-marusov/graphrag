...
> **Текущее состояние проекта:** этап видения — интерфейс и кодовая база отсутствуют (`../src/README.md`); всё, что касается прогонов, стендов и CI, описывает **целевое состояние**; состав экранов веб-интерфейса фиксируется в UI-дизайн-артефактах Фазы 1 до начала реализации.  
> **Методологическая база:** спецификации `specs/qa/*.md` (E2E GUI, граница с E2E API, unit/integration, гейты, план тестирования), методология W&B (Validation, acceptance criteria, traceability), навыки `../../.agents/skills/aif-qa/SKILL.md` и `../../.agents/skills/quality-matrix/SKILL.md`; `../vision.md` (функции F1–F4, режимы, роли, метрики, эталонные кейсы K1/K4/K5/K17) — контекст для шагов, ожидаемых состояний и assertions; каталоги требований (функциональных, use cases, бизнес-правил, НФТ, пользовательских историй) — планируются (vision.md §2.2); код `../src/` — только для понимания реализации, не источник правды.

---

## 1. Место E2E GUI-тестов в стратегии качества

E2E GUI-тест проверяет **полный сквозной путь пользователя через веб-интерфейс**: действие пользователя (ввод запроса, навигация, клик) → обработка реальными процессами контура GraphRAG (ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, контур LLM) → наблюдаемый пользователем результат (ответ со ссылками на источники, статус индексации, метрики, событие аудита). Тест исполняется в **браузере пользователя** (desktop): веб-интерфейс системы знаний, а не сервис.
...
**Правило источников.** E2E GUI-сценарии выводятся **только из зафиксированных пользовательских сценариев**: на этапе видения — из функций `../vision.md` (F-коды F1.1–F4, §2.2) и эталонных кейсов (K1, K4, K5, K17); после появления каталога пользовательских историй (Gherkin) он станет основным источником. Роли, режимы, НФТ-метрики и контракты — контекст: они уточняют шаги, ожидаемые состояния и assertions, но не порождают E2E GUI-сценарии. Если для пути нет пользовательского сценария — уровень не E2E GUI, а GUI-integration (до появления требований), сценарий помечается `†` (раздел 12).
...
|---|---|
| Основная база | каталог пользовательских историй (Gherkin) появляется в `specs/user-stories/` по мере фиксации, vision.md §2.2; `../vision.md` (F1–F4, режимы, роли, метрики), `../glossary.md`, `../../.ai-factory/RESEARCH.md`, `../../.ai-factory/PLAN.md` — контекст; спецификации `specs/qa/*.md` — методология |
| Объект | целевой веб-интерфейс системы знаний (состав экранов фиксируется в UI-дизайн-артефактах Фазы 1 до начала реализации): режим вопросов/ответов со ссылками на источники (F1), администрирование (индексация F2, онтологический слой, метрики), аудит контура LLM (REQ-NFR-security.compliance.llm-contour); a11y |
...
**Граница с integration:** если проверяется один стык (веб-интерфейс ↔ API системы знаний, движок запросов ↔ хранилище графа) с контролем середины пути — это integration (`specs/qa/integration.md`); проверка внутренних состояний без пользовательского сценария — тоже integration.  
...
|---|---|
| P0 | Критический GUI-путь (эталонные кейсы K1, функции F1, сквозное требование REQ-NFR-security.compliance.llm-contour): пользователь задаёт вопрос → ответ со ссылками на источники; администратор запускает индексацию → статус и метрики обновляются; ИБ просматривает аудит обращений к контуру LLM; корпоративная аутентификация и аудит действий (целевой сценарий) |
| P1 | Основные пользовательские пути: администратор загружает обновлённую модель предметной области → пересчёт резолвинга (онтологический слой); сверка ответа с источником (K5); контекстный режим через MCP (F4, K17); a11y-основа (screen reader, keyboard-only) |
| P2 | Отображение деталей (источники, метаданные артефактов), тёмная тема, трассируемость/анализ влияния (F3), детальные отчёты метрик |

### 3.2. Объекты E2E GUI-тестирования

| Объект | Что проверять сквозным пользовательским путём |
|---|---|
| Режим вопросов/ответов (F1) | (целевой, состав экранов фиксируется в UI-дизайн-артефактах Фазы 1 до начала реализации): ввод запроса → ответ со ссылками на источники (grounding/provenance), переход к источнику, пустые/неоднозначные результаты |
| Администрирование (F2, онтологический слой) | (целевой): запуск/статус индексации, метрики индекса, онтологический слой (механизм F1/F3), сообщения об ошибках |
| Аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | (целевой): события обращений к LLM, фильтрация утечек (REQ-NFR-security.compliance.llm-contour), журнал аудита, понятные сообщения |
...
Источник правды для E2E GUI-сценария — **зафиксированный пользовательский сценарий**: сценарий существует, потому что есть пользовательское поведение, которое нужно проверить через интерфейс. На этапе видения основной источник — **`../vision.md`**: функции F1–F4 (§2.2), режимы (§1.5/§2.1), роли (§3.1), метрики и эталонные кейсы (K1, K4, K5, K17). Остальные артефакты — контекст: они уточняют шаги, ожидаемые состояния и assertions, но не порождают E2E GUI-сценарии.

**Основной источник (правило отбора):**

1. **`../vision.md`** — функции F1–F4 с подфункциями (F1.1, F1.2, F2.1, F2.2, F3.1, F3.2, F4; §2.2), режимы продукта, роли, метрики; каталог пользовательских историй (Gherkin) **планируется** (vision.md §2.2) и станет основным источником E2E GUI-сценариев. Пока каталога нет, сценарии помечаются `†` и исполняются на GUI-integration-уровне (разделы 4 и 12).

> **Текущее состояние:** проект на этапе видения — интерфейс и кодовая база отсутствуют (`../src/README.md`); каталог пользовательских историй не создан. Все GUI-сценарии — целевые, а состав экранов фиксируется в UI-дизайн-артефактах Фазы 1 до начала реализации и затем транслируется в сценарии; помечены `†`/`*` и до появления требований исполняются на GUI-integration-уровне или ожидают этапа проектирования (разделы 4 и 12). Появление каталога требований расширяет E2E GUI-набор и является предусловием для гейта трассируемости (FG-TRACE).

**Контекст и детали:**

2. **`../vision.md`** — функции F1–F4 (vision §2.2): F1 генерация проверяемых ответов, F2 индексация источников знаний, F3 оптимизация контекста для автоматической разработки, F4 доставка через MCP — критерии целевого поведения; механизмы (онтологический слой, контур LLM — REQ-NFR-security.compliance.llm-contour) функциями не являются; каталоги требований (функциональных, use cases, бизнес-правил) — планируются (vision.md §2.2).
3. **Роли и каналы** — vision §3.1: разработчик (быстрый доступ через IDE/MCP), аналитик, тестировщик, администратор (индексация, метрики, контур LLM), ИБ (аудит контура LLM), менеджер проекта; взаимодействие через API/MCP.
4. **Контракты интерфейсов** — фиксируются на этапе проектирования (Фаза 1): веб-интерфейс ↔ API системы знаний; MCP-сервер (F4); Git API/webhook/MR (GitLab).
5. **НФТ-метрики** — vision §2.2: точность ответов RAG ≥ 70 %, время ответа (медиана, p90), число противоречий, доля артефактов со связями, доля обращений к LLM через корпоративный канал (0 вне контура), минимизация токенов.
...
Правило для ИИ-агента: **не выдумывать пользовательские сценарии, элементы UI и детали контракта**; они берутся только из зафиксированных требований и UI/ API/ MCP-артефактов Фазы 1. Если требование и предполагаемая реализация расходятся, не «чинить» сценарий молча — зафиксировать расхождение как дефект, открытый вопрос или known gap.
...
2. Найти связанные F-коды (F1.1–F4), роли (vision §3.1), режимы (vision §1.5/§2.1) и НФТ-метрики для контекста: шаги, ожидаемые состояния, ограничения.
...
4. Определить сквозной путь: пользователь → элемент веб-интерфейса → сегменты (API системы знаний → движок запросов / пайплайны индексации → хранилище графа / контур LLM / GitLab) → наблюдаемый пользовательский результат.
5. Найти контракт сегмента, зафиксированный в UI/ API/ MCP-артефактах Фазы 1, и внутренние состояния системы за интерфейсом.
...
7. Разделить проверки по уровням:
   ...
   - стык (веб-интерфейс ↔ API, движок запросов ↔ хранилище) с контролем середины → integration;
...
```text
Требование (основной источник): <F-код F1.1–F4 / эталонный кейс K1/K4/K5/K17> (vision §2.2;
  Gherkin-история появится в каталоге требований — планируется)
Контекст: <роль (vision §3.1), режим (vision §1.5/§2.1), НФТ-метрика>
Сценарий QA: <UI-01…UI-04 / UI-A11Y-*> (уровень E2E GUI; состав экранов фиксируется в UI-дизайн-артефактах Фазы 1)

Сквозной путь:
- initiator: <пользователь в браузере (desktop) / MCP-клиент>
- segments: <элемент веб-интерфейса → API системы знаний → движок запросов / пайплайны индексации
...
- <реальные процессы контура GraphRAG: ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, интеграции>
...
```python
# F1 (vision §2.2): пользователь задаёт вопрос и получает ответ со ссылками на источники (K1).
# F2 (vision §2.2): администратор запускает индексацию и видит статус и метрики (контекст).
# Онтологический слой (механизм F1/F3): администратор загружает обновлённую модель предметной области, резолвинг пересчитывается (контекст).
...
Состав экранов не зафиксирован (этап видения); целевой веб-интерфейс: режим вопросов/ответов (F1), администрирование (индексация F2, онтологический слой, метрики), аудит контура LLM (REQ-NFR-security.compliance.llm-contour) — контракт UI фиксируется в UI-дизайн-артефактах Фазы 1.

| Область интерфейса | Реализация | Ключевые состояния/значения | Сценарии |
|---|---|---|---|
| Состав экранов | не зафиксирован (этап видения; `../src/README.md`) | целевой веб-интерфейс: режим вопросов/ответов (F1), администрирование (F2, онтологический слой), аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | UI-01…UI-04 |
| Контракт UI | фиксируется в UI-дизайн-артефактах Фазы 1 | ключевые элементы, состояния и значения определяются контрактом UI | UI-01…UI-04 |
| Режим вопросов/ответов (F1) | фиксируется в UI-дизайн-артефактах Фазы 1 | ввод запроса, ответ со ссылками на источники (grounding), переход к источнику, состояния «Загрузка...»/ошибка | UI-01 |
| Администрирование (F2, онтологический слой) | фиксируется в UI-дизайн-артефактах Фазы 1 | запуск/статус индексации, метрики индекса, онтологический слой (механизм F1/F3), состояния ошибок | UI-02, UI-03 |
| Аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | фиксируется в UI-дизайн-артефактах Фазы 1 | события обращений к LLM, фильтрация утечек (REQ-NFR-security.compliance.llm-contour), журнал аудита | UI-04 |
| API/MCP-стык | фиксируется в UI/ API/ MCP-артефактах Фазы 1 | веб-интерфейс ↔ API системы знаний; MCP-сервер (F4) — для ИИ-агентов | фон всех UI-* |
...
|---|---|---|
| Запрос → ответ (K1) | пользователь → ввод запроса → API → движок запросов → граф/онтология → контур LLM → экран | ответ со ссылками на источники; источник реально существует и соответствует grounding |
| Индексация (F2) | администратор → запуск индексации → пайплайны → хранилище графа → экран | статус индексации и метрики обновляются; изменения GitLab (webhook/MR) подхватываются |
| Онтология (онтологический слой) | администратор → загрузка обновлённой модели → пересчёт резолвинга → экран | сигнал о пересчёте; резолвинг пересчитан (онтологический слой) |
| Аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | ИБ → открытие аудита → события контура → экран | события обращений к LLM, фильтрация утечек (REQ-NFR-security.compliance.llm-contour); вне контура — 0 обращений |
| Контекстный режим (F4, K17) | ИИ-агент → MCP → context compiler → контур | контекст собран и передан агенту (проверяется в E2E API; в GUI — отображение/статус) |
...
- проверка одного API-стыка (веб-интерфейс ↔ API системы знаний, движок запросов ↔ хранилище) с контролем середины (integration);
...
| Основной стенд интеграции | Реальные процессы контура GraphRAG (ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, интеграции) + точечные заглушки внешних стыков | integration-сценарии, в т. ч. GUI-integration (UI-* до появления требований) |
...
- **Реальные процессы контура GraphRAG**: ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, интеграции — реальные процессы; ни один проверяемый компонент не заменён эмулятором.
...
- Стенд допускает fault injection: остановку компонентов контура (движок запросов, MCP-сервер, контур LLM), отключение сети, смену темы/масштаба для негативных и НФТ-сценариев.
...
### 7.4. Процедура a11y-проверок (WCAG 2.1 AA)

Автоматическая проверка доступности — **axe-core** с набором тегов `['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa']` (решение владельца продукта 2026-08-30; базовое правило — `BR-constraint.web-app-browser-chat`).

**Обязательные критерии A/AA:**

| Категория | WCAG-критерий | Что проверяется |
|---|---|---|
| Воспринимаемость | 1.1.1 | Наличие `alt` у изображений |
| | 1.3.1 | Структура заголовков, семантическая разметка |
| | 1.4.3 | Контраст текста ≥ 4.5:1 (обычный), ≥ 3:1 (крупный) |
| | 1.4.11 | Контраст не-текстовых элементов ≥ 3:1 |
| | 1.3.5 | `autocomplete` на полях персональных данных |
| Управляемость | 2.1.1 | Вся функциональность доступна с клавиатуры |
| | 2.1.2 | Нет ловушек клавиатуры (focus trap) |
| | 2.4.3 | Логичный порядок фокуса (Tab) |
| | 2.4.7 | Видимый индикатор фокуса |
| Понятность | 3.3.2 | Доступное имя (label) у всех полей форм |
| Надёжность | 4.1.2 | Доступные имена интерактивных элементов; валидные ARIA-атрибуты, `aria-hidden` не блокирует фокус |

Критерии AAA — опциональны (не обязательны для приёмки): 1.4.6 (контраст 7:1/4.5:1), 2.2.1 (настраиваемые таймауты), 2.2.2 (отключение анимации/мигания), 2.4.8 (местоположение), 2.4.9/2.4.10 (ссылки и заголовки по тексту), 2.5.3 (доступное имя по тексту), 3.2.5 (изменение контекста по требованию), 3.3.5/3.3.6 (подсказки и предотвращение ошибок).

**Pass/Fail-критерий:**

| Уровень нарушений | Действие |
|---|---|
| Critical / Serious | ❌ Блокирующие: тест FAIL, релиз невозможен |
| Moderate | ⚠️ Предупреждение: PASS при зарегистрированных Issues в трекере |
| Minor | ℹ️ Информация: PASS |

**Ручные проверки (не автоматизируемое):** навигация с клавиатуры (Tab-порядок, видимый фокус, отсутствие ловушек); динамические компоненты (модальные окна, dropdown, раскрывающиеся панели); контраст элементов с цветом из CSS; экраны после действия (отправка формы, выполнение операции); интерактивные диаграммы (Mermaid) — клавиатурная доступность и текстовые альтернативы.

**Инструменты:** axe-core (Playwright/Cypress) — автоматика в CI; `wcag-checker` — сканирование файлов с HTML-отчётом; браузерные DevTools — локальная отладка; Screen Reader (VoiceOver/NVDA) — ручное тестирование динамического контента.

**CI-интеграция:** запуск a11y-тестов в пайплайне (например, `npx pw-a11y test` с `PW_A11Y_FAIL=1` и тегами `wcag2a,wcag2aa,wcag21a,wcag21aa`); гейт — Critical/Serious блокируют релиз.
...
- проверяем «запрос → ответ со ссылками» → реальные интерфейс, движок запросов и хранилище графа, тестовый контур (эмулятор LLM с golden-ответами) как источник состояния;
...
- проверяем «аудит контура LLM (REQ-NFR-security.compliance.llm-contour)» → реальные интерфейс и контур LLM (эмулятор), события аудита как наблюдатель.
...
|---|---|
| Веб-интерфейс | целевой веб-интерфейс системы знаний (браузер пользователя); состав экранов фиксируется в UI-дизайн-артефактах Фазы 1 |
| Ядро / оркестрация пайплайнов | индексация корпуса (F2), статусы и метрики |
| Движок запросов | обработка запросов, ответы со ссылками на источники (F1, K1) |
| Онтологический слой (механизм F1/F3) | машинно-читаемая модель предметной области, резолвинг (механизм F1/F3) |
| MCP-сервер | контекстный режим для ИИ-агентов (F4, K17) |
...
|---|---|
| Запрос → ответ (UI-01) | веб-интерфейс + движок запросов + хранилище графа + эмулятор LLM (golden-ответы) |
| Индексация (UI-02) | веб-интерфейс + пайплайны индексации + хранилище графа + заглушка GitLab |
| Онтология (UI-03) | веб-интерфейс + онтологический слой (механизм F1/F3) + хранилище графа |
...
| Контекстный режим MCP (F4, K17) | MCP-сервер + движок запросов + контур LLM (проверяется в E2E API; в GUI — статус/отображение) |
...
2. Проверить, что integration-сценарии по затронутым стыкам зелёные (веб-интерфейс ↔ API, движок запросов ↔ хранилище и др.).
3. Проверить контракты API/MCP по мере появления UI/ API/ MCP-артефактов Фазы 1; на этапе видения контрактов нет, поэтому это входной критерий будущего E2E-прогона, а не открытый вопрос требований.
...
2. Установить компоненты контура GraphRAG (ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, интеграции), открыть веб-интерфейс в браузере пользователя.
3. Проверить deployment-smoke:
   - веб-интерфейс отвечает, страница загружается;
   - навигация по интерфейсу работает;
   - контур GraphRAG отвечает (движок запросов, MCP-сервер, контур LLM);
...
- **API-контракт** (веб-интерфейс ↔ API системы знаний; фиксируется в API-артефактах Фазы 1): ключевые операции отвечают по контракту; таймауты, status/error mapping корректны — проверяется внешним API-клиентом (это conformance, не E2E GUI);
- **UI-контракт**: ключевые элементы страницы доступны через DOM/a11y-дерево с ожидаемыми ролями/именами; скриншот-эталон основных экранов снят после фиксации состава экранов в UI-дизайн-артефактах Фазы 1;
...
3. **Failure injection** — остановлен компонент контура (движок запросов, MCP-сервер, контур LLM), отключена сеть → контролируемое состояние GUI, без падения.
...
- неавторизованный доступ к интерфейсу/настройкам отклоняется; аудит обращений к контуру LLM пишется (REQ-NFR-security.compliance.llm-contour);
- невалидный ввод (запрос, параметры индексации) → явная ошибка, состояние не ломается;
- контур LLM/движок запросов недоступен → интерфейс показывает понятное состояние без падения (только негативные сценарии);
...
| Ядро / оркестрация пайплайнов | логи пайплайнов индексации | статусы индексации (F2), события GitLab (webhook/MR) |
| Движок запросов | логи движка запросов | запросы, ответы, grounding (F1, K1) |
| MCP-сервер | логи MCP-сервера | сессии ИИ-агентов (F4, K17) |
| Контур LLM | логи шлюза LLM | обращения к LLM, фильтрация утечек (REQ-NFR-security.compliance.llm-contour) |
...
Ниже — сквозные проверки по целевым областям веб-интерфейса и смежным доменам. Состав экранов не зафиксирован (этап видения); все сценарии — целевые, а состав экранов фиксируется в UI-дизайн-артефактах Фазы 1 и затем транслируется в сценарии. Сценарии `†` (нет зафиксированного пользовательского сценария) исполняются на GUI-integration-уровне до появления требований; `*` — целевая модель (to be).

### 11.1. Режим вопросов/ответов (F1)

E2E GUI-проверки (целевые; состав экранов фиксируется в UI-дизайн-артефактах Фазы 1):
...
### 11.2. Администрирование (F2, онтологический слой)

E2E GUI-проверки (целевые; состав экранов фиксируется в UI-дизайн-артефактах Фазы 1):

- администратор запускает индексацию корпуса → статус индексации и метрики обновляются (UI-02, F2);
- изменения в GitLab (коммит/webhook/MR) подхватываются индексацией (F2);
- администратор загружает обновлённую модель предметной области → сигнал и пересчёт резолвинга (UI-03, онтологический слой);
- отображаются метрики: доля артефактов со связями, число противоречий, доля вопросов, закрытых без аналитика;
- состояние «Загрузка...» и понятная ошибка при сбое индексации.

Обязательные negative cases: некорректные параметры индексации, недоступный GitLab, конфликтующая модель предметной области, повторный запуск индексации (без дублирования эффекта).

### 11.3. Авторизация (SSO)

E2E GUI-проверки (`BR-constraint.sso-readonly-access`, `ADR-DES.SECURITY.sso-keycloak`; состав экранов фиксируется в UI-дизайн-артефактах Фазы 1):

- доступ к интерфейсу/настройкам разрешён только авторизованным пользователям; попытка без корпоративной аутентификации (SSO Keycloak) отклоняется с понятным сообщением;
- истёкший/невалидный OIDC-токен → перенаправление на вход SSO или понятная ошибка (входит в MVP);
- единый пользователь по умолчанию, ролей нет (read-only).

Обязательные negative cases: неавторизованная попытка, невалидный/истёкший токен SSO — дефект.

### 11.4. Аудит контура LLM (REQ-NFR-security.compliance.llm-contour)

E2E GUI-проверки (состав экранов фиксируется в UI-дизайн-артефактах Фазы 1):

- ИБ открывает аудит обращений к контуру LLM → события фильтрации и аудита (UI-04, REQ-NFR-security.compliance.llm-contour/REQ-NFR-api.performance.token-minimization);
...
Обязательные negative cases: отсутствие записи аудита при выполненном действии — дефект.

### 11.5. Доступность (a11y)

E2E GUI-проверки (уровень **WCAG 2.1 AA**, `BR-constraint.web-app-browser-chat`; сценарии обязательны):
...
- порядок фокуса и контраст соответствуют WCAG 2.1 AA;
- локализация: переключение русский/английский, отсутствие жёстко зашитых строк, корректное отображение обоих языков (i18n-архитектура, возможность добавления локалей).

До появления реализации (этап видения) сценарии a11y (`UI-A11Y-*`) исполняются как known gap / expected fail и фиксируются в разделе «Открытые вопросы»; после появления UI — обязательны (требования зафиксированы).

---

## 12. Каталог E2E GUI-сценариев

Полные описания — в `specs/qa/test-plan.md`. Ниже свод идентификаторов с уровнем `E2E GUI` (все — целевые; состав экранов фиксируется в UI-дизайн-артефактах Фазы 1).

| Домен/область | Идентификаторы сценариев | Требование-источник |
|---|---|---|
| Режим вопросов/ответов (F1, сквозной) | UI-01† | F1; эталонный кейс K1 (vision §2.2) |
| Администрирование: индексация источников знаний (F2) | UI-02† | F2 (vision §2.2) |
| Администрирование: онтологический слой (механизм F1/F3) | UI-03† | механизм (vision §2.5) |
| Аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | UI-04† | REQ-NFR-security.compliance.llm-contour, REQ-NFR-api.performance.token-minimization (vision §2.5) |
...
| Онтология | тестовые изменения модели предметной области: валидные/невалидные, конфликтующие |
...
- Скриншоты <answer_01.png>…<answer_03.png> и a11y-дерево приложены; логи движка запросов и контура LLM — артефакты.
```
...
3. `../vision.md` (F1–F4), `../glossary.md`, ADR — контекст и критерии;
...
- контур LLM/движок запросов недоступен → контролируемое состояние ошибки, без падения;
...
| Внешний контур | нестабильный эмулятор LLM/заглушка GitLab | задержки ответов движка запросов |
...
| 2. Привязка | «Сценарий не про то требование» | trace-комментарий `// F1: ...` (раздел 5.3) | `FG-TRACE` |
...
- при остановке компонента контура (движок запросов, контур LLM) он падает ожидаемо;
...
Если в репозитории доступна команда проверки контрактов — выполнить её перед E2E-прогонами; иначе использовать каноническую команду из §17.1.
...
- Playwright используется выборочно, для критичных smoke/regression сценариев: открытие интерфейса, навигация, запрос/ответ (K1), индексация (F2), аудит (REQ-NFR-security.compliance.llm-contour), keyboard-only;
...
|---|---|---|
| `UI-01` (запрос/ответ, K1), `UI-04` (аудит, REQ-NFR-security.compliance.llm-contour) | Go integration + выборочно Playwright smoke | Основную логику дешевле и стабильнее проверять ниже E2E; браузерный smoke подтверждает реальную страницу |
...
- [ ] указаны контракты и версии сегментов пути (по мере появления; на этапе видения — пометка «фиксируется в UI/ API/ MCP-артефактах Фазы 1»);
...
1. `../vision.md` — функции F1–F4, режимы (§1.5/§2.1), роли (§3.1), метрики (§2.2), эталонные кейсы (K1, K4, K5, K17); каталог пользовательских историй (Gherkin) планируется — до появления сценарии `†`;
...
Если данных недостаточно — сформулируй вопрос. Не выдумывай поведение UI и контракт; состав экранов и контракты берутся только из зафиксированных артефактов Фазы 1.
...
```text
Scenario ID: <ID>
Trace: <F-код F1.1–F4 / эталонный кейс> (основной); <роль, режим, НФТ-метрика> (контекст)
Path:
- initiator: <пользователь в браузере (desktop) / MCP-клиент>
- segments: <элемент веб-интерфейса → API → движок запросов / пайплайны индексации
            → хранилище графа / контур LLM / GitLab>
...
Processes: real <ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, ...>;
...
- Real processes: `<ядро, оркестрация пайплайнов, движок запросов, MCP-сервер, контур LLM, ...>`
...
sequenceDiagram
    ...
    participant QE as Движок запросов / пайплайны
...
- **R3 (этап видения):** интерфейс и кодовая база отсутствуют; все GUI-сценарии — целевые, состав экранов фиксируется в UI-дизайн-артефактах Фазы 1; CI-покрытие E2E GUI — целевое состояние.
...
- **R13 (интерфейс не зафиксирован):** состав экранов фиксируется в UI-дизайн-артефактах Фазы 1; сценарии UI-01…UI-04 отражают целевую модель (expected fail / known gap).
...
- **K1/REQ-NFR-security.compliance.llm-contour (P0):** проверка «ответ со ссылками на источники» и «аудит контура LLM» требует согласованного стенда (реальные процессы контура + тестовый контур с golden-ответами) — дефект в этом классе блокирует релиз.
...
- `../vision.md` — функции F1–F4, режимы (§1.5/§2.1), роли (§3.1), метрики (§2.2), эталонные кейсы (K1, K4, K5, K17) — основной источник требований на этапе видения.
