...
| P1 | Основные пользовательские пути: администратор загружает обновлённую модель предметной области → пересчёт резолвинга (F2.3); сверка ответа с источником (K5); обработка сигналов с полей (F5); контекстный режим через MCP (F6, K17); a11y-основа (screen reader, keyboard-only) |
...
# F2.3 (vision §2.2): администратор загружает обновлённую модель предметной области, резолвинг пересчитывается (контекст).
...
| Онтология (F2.3) | администратор → загрузка обновлённой модели → пересчёт резолвинга → экран | сигнал о пересчёте; резолвинг пересчитан (F2.3) |
...
| Онтологический слой (F2) | машинно-читаемая модель предметной области, резолвинг (F2.3) |
...
- администратор загружает обновлённую модель предметной области → сигнал и пересчёт резолвинга (UI-03, F2.3);
