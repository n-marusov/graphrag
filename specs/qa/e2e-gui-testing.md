...
| Объект | целевой веб-интерфейс системы знаний (состав экранов уточняется на этапе проектирования): режим вопросов/ответов со ссылками на источники (F1), администрирование (индексация F2, онтологический слой, метрики), аудит контура LLM (NFR-SEC-1); a11y |
...
|---|---|
| P0 | Критический GUI-путь (эталонные кейсы K1, функции F1, сквозное требование NFR-SEC-1): пользователь задаёт вопрос → ответ со ссылками на источники; администратор запускает индексацию → статус и метрики обновляются; ИБ просматривает аудит обращений к контуру LLM; корпоративная аутентификация и аудит действий (целевой сценарий) |
...
| Аудит контура LLM (NFR-SEC-1) | (целевой): события обращений к LLM, фильтрация утечек (NFR-SEC-1), журнал аудита, понятные сообщения |
...
Состав экранов не зафиксирован (этап видения); целевой веб-интерфейс: режим вопросов/ответов (F1), администрирование (индексация F2, онтологический слой, метрики), аудит контура LLM (NFR-SEC-1) — контракт UI уточняется при проектировании.

| Область интерфейса | Реализация | Ключевые состояния/значения | Сценарии |
|---|---|---|---|
| Состав экранов | не зафиксирован (этап видения; `../src/README.md`) | целевой веб-интерфейс: режим вопросов/ответов (F1), администрирование (F2, онтологический слой), аудит контура LLM (NFR-SEC-1) | UI-01…UI-04 |
...
| Аудит контура LLM (NFR-SEC-1) | уточняется на этапе проектирования | события обращений к LLM, фильтрация утечек (NFR-SEC-1), журнал аудита | UI-04 |
...
| Аудит контура LLM (NFR-SEC-1) | ИБ → открытие аудита → события контура → экран | события обращений к LLM, фильтрация утечек (NFR-SEC-1); вне контура — 0 обращений |
...
- проверяем «аудит контура LLM (NFR-SEC-1)» → реальные интерфейс и контур LLM (эмулятор), события аудита как наблюдатель.
...
- неавторизованный доступ к интерфейсу/настройкам отклоняется; аудит обращений к контуру LLM пишется (NFR-SEC-1);
...
| Контур LLM | логи шлюза LLM | обращения к LLM, фильтрация утечек (NFR-SEC-1) |
...
### 11.3. Аудит контура LLM (NFR-SEC-1)

E2E GUI-проверки (целевые; состав экранов уточняется на этапе проектирования):

- ИБ открывает аудит обращений к контуру LLM → события фильтрации и аудита (UI-04, NFR-SEC-1/NFR-PERF-1);
...
| Аудит контура LLM (NFR-SEC-1) | UI-04† | NFR-SEC-1, NFR-PERF-1 (vision §2.5) |
...
- Playwright используется выборочно, для критичных smoke/regression сценариев: открытие интерфейса, навигация, запрос/ответ (K1), индексация (F2), аудит (NFR-SEC-1), keyboard-only;
...
|---|---|---|
| `UI-01` (запрос/ответ, K1), `UI-04` (аудит, NFR-SEC-1) | Go integration + выборочно Playwright smoke | Основную логику дешевле и стабильнее проверять ниже E2E; браузерный smoke подтверждает реальную страницу |
...
- **K1/NFR-SEC-1 (P0):** проверка «ответ со ссылками на источники» и «аудит контура LLM» требует согласованного стенда (реальные процессы контура + тестовый контур с golden-ответами) — дефект в этом классе блокирует релиз.
