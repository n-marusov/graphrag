...
|---|---|
| Основная база | каталог пользовательских историй (планируется, vision.md §2.2); `../vision.md` (F1–F4, режимы, роли, метрики), `../glossary.md`, `../../.ai-factory/RESEARCH.md`, `../../.ai-factory/PLAN.md` — контекст; спецификации `specs/qa/*.md` — методология |
| Объект | целевой веб-интерфейс системы знаний (состав экранов уточняется на этапе проектирования): режим вопросов/ответов со ссылками на источники (F1), администрирование (индексация F2, онтологический слой, метрики), аудит контура LLM (REQ-NFR-security.compliance.llm-contour); a11y |
...
|---|---|
| Режим вопросов/ответов (F1) | (целевой, состав экранов уточняется на этапе проектирования): ввод запроса → ответ со ссылками на источники (grounding/provenance), переход к источнику, пустые/неоднозначные результаты |
...
4. **Контракты интерфейсов** — уточняются на этапе проектирования (Фаза 1): веб-интерфейс ↔ API системы знаний; MCP-сервер (F4); Git API/webhook/MR (GitLab).
...
Состав экранов не зафиксирован (этап видения); целевой веб-интерфейс: режим вопросов/ответов (F1), администрирование (индексация F2, онтологический слой, метрики), аудит контура LLM (REQ-NFR-security.compliance.llm-contour) — контракт UI уточняется при проектировании.
...
| Режим вопросов/ответов (F1) | уточняется на этапе проектирования | ввод запроса, ответ со ссылками на источники (grounding), переход к источнику, состояния «Загрузка...»/ошибка | UI-01 |
| Администрирование (F2, онтологический слой) | уточняется на этапе проектирования | запуск/статус индексации, метрики индекса, онтологический слой (механизм F1/F3), состояния ошибок | UI-02, UI-03 |
| Аудит контура LLM (REQ-NFR-security.compliance.llm-contour) | уточняется на этапе проектирования | события обращений к LLM, фильтрация утечек (REQ-NFR-security.compliance.llm-contour), журнал аудита | UI-04 |
| API/MCP-стык | уточняется на этапе проектирования | веб-интерфейс ↔ API системы знаний; MCP-сервер (F4) — для ИИ-агентов | фон всех UI-* |
...
E2E GUI-проверки (`BR-constraint.sso-readonly-access`, `ADR-DES.SECURITY.sso-keycloak`; состав экранов уточняется на этапе проектирования):
...
E2E GUI-проверки (состав экранов уточняется на этапе проектирования):
