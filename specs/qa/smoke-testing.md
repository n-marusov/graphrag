...
```mermaid
flowchart LR
    A[Требование / потребность] --> B[Функции F1–F7 / метрики / NFR / контрактные кейсы]
    B --> C[Реализация]
    C --> D[Build smoke]
    D --> E[Install smoke]
...
Smoke даёт данные для gate-решения. US (в GraphRAG — функции F1–F7, `../vision.md` §2.1–2.2) описывает ценность и, если касается критического пути, может усиливать трассировку, но не является источником smoke-набора.
...
1. **P0 FR / функции F1–F7** — критические функциональные требования (`../vision.md` §2.1–2.2);
...
2. Найти функции F1–F7, метрики, NFR и соответствующие контрактные кейсы (K1, K4, K5, K17).
...
US не является обязательным источником smoke. Если US существует и лежит на критическом пути, он может усилить трассировку, но не определяет smoke-набор. На этапе видения каталоги требований планируются (`../vision.md` §2.2), поэтому smoke опирается на функции F1–F7, метрики и эталонные кейсы K1, K4, K5, K17.
...
- scenario ID;
- trace на функции F1–F7, метрики и контрактные кейсы;
...
| SM-D-09 | Онтологический слой загружен, резолвинг по модели предметной области работает (F2) | P1 |
| SM-D-10 | Сигналы с полей импортируются (F5) | P1 |
...
### 7.4. Функциональные акценты (F1–F7)

- `F1 индексация`: SM-D-02, SM-D-11, SM-D-12
- `F2 онтологический слой`: SM-D-09
- `F3 ответы с grounding`: SM-D-03, SM-D-06
- `F4 анализ влияния`: SM-D-05
- `F5 накопление знаний`: SM-D-10
- `F6 context compiler`: SM-D-04
- `F7 безопасный контур LLM`: SM-D-08
...
- есть trace на функции F1–F7, метрики и контрактные кейсы;
...
- [ ] Трассировка идёт от функций F1–F7, метрик и контрактных кейсов.
...
- `../vision.md` — видение: цели G1–G7 (§1.3), функции F1–F7 (§2.1–2.2), события (§2.4), внешние зависимости (§2.8)
