...
- старт сервисов: ядро, оркестрация пайплайнов, движок запросов, MCP-сервер;
...
- CLI отвечает на запрос версии продукта (бинарник на стенде жизнеспособен);
...
- старт сервисов: ядро, пайплайны, движок запросов, MCP-сервер;
...
| SM-B-09 | Нет секретов и запрещённых зависимостей (детекция секретов/ПДн/ключей в репозитории и артефактах; политика лицензий — BR-constraint.opensource-only) | CI | P1 |
| SM-B-10 | Эмулятор LLM и заглушки тестового контура собираются | CI | P2 |
| SM-B-11 | Бинарник `graphrag` собирается, запускается и отвечает версией продукта | CI | P1 |
...
| SM-I-04 | Сервисы развёрнуты: ядро, пайплайны, движок запросов, MCP-сервер | dev-стенд | P0 |
...
| SM-D-10 | CLI возвращает версию продукта (`graphrag --version`) — бинарник на стенде жизнеспособен | P0 |
...
- `REQ-NFR-security.compliance.llm-contour безопасный контур LLM`: SM-D-08
- `здоровье/метрики`: SM-D-07

### 7.5. Базовый набор

Обязательный минимум deployment smoke:
SM-D-02, SM-D-03, SM-D-04, SM-D-05, SM-D-06, SM-D-07, SM-D-08, SM-D-10.
...
# 4. старт сервисов: ядро, пайплайны, движок запросов, MCP-сервер

# deployment smoke (команда определяется на этапе проектирования)
# базовый набор: SM-D-02, SM-D-03, SM-D-04, SM-D-05, SM-D-06, SM-D-07, SM-D-08, SM-D-10, SM-D-11, SM-D-12
# CLI (SM-D-10): graphrag --version — ожидается вывод версии продукта (команда уточняется на этапе проектирования)
```
...
- `SCA/лицензии open-weight моделей`, соответствие 152-ФЗ (ПДн не обрабатываются системой — vision §2.7 п.12) и часть `security` — целевая модель, могут исполняться как expected fail / known gap.
...
- `../business-rules/BR-constraint.backend-cli-queries.md` — CLI как канал запросов к системе знаний (не инструмент администрирования)
